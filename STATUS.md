...
- Repository logic gate (`-m "not real_model"`): 5921 passed, 13 skipped,
...
- Sherpa carries immutable acoustic identity through partial, final, barge,
  ...
  final, and abort replay. Typed stale/duplicate ingress is rejected before effects; command PCM cannot also become a normal ASR final; post-barge grants
  require matching acoustic keys; abort rollback affects only its partial. An
  accepted revision continues through task, TTS, and playback (ADR-0084/0086).
...
- Synchronous capabilities use bounded cancellable coordinators. Mutating
  ...
  idempotency. Timeouts fence unstarted providers; provider-started calls ignore barge but see explicit/shutdown cancellation. Tools do not retry
...
- Port the session actor contract and golden lifecycle cases to remote/mobile
  brains; bound the mailbox and targeted cancellation before claiming parity.
