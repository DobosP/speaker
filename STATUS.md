...
- Current non-real logic gate: 7,801 passing checks as 7,800 low-priority host-side broad passes plus the bounded-read race isolated; 14 skipped, 24 model-only deselected, 11 warnings. The repository lock passed in broad mode 600. Candidate/shared focused gate: 464 + 1 isolated; broad streaming 966; public command/noise focused 246; current APM/DTD 6.
...
- Keep parakeet.cpp isolated while adding held-out conversation/Common Voice comparisons, multi-host build receipts, endpoint-ground-truth checks, and separate live capture validation; do not change runtime defaults until command safety, ownership, device, and live gates pass. Continue publisher/mobile/remote work separately (ADR-0099/0118/0119).
