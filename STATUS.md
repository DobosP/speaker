...
- Current non-real logic gate: 7,617 passing checks as 7,615 low-priority host-side broad passes plus two exact-condition reruns (repository lock mode 600 and the bounded-read race isolated); 14 skipped, 24 model-only deselected, 11 warnings. Final-command FileReplay/adjacent gate: 146, 2 skipped; public command/noise focused gate: 276 + 1 isolated; current APM/DTD 6.
...
- Keep the next receipt-bound `parakeet.cpp` experiment, Common Voice/Faster-Whisper model runs, and publisher/mobile/remote work isolated until their capture, ownership, device, and live gates pass (ADR-0099/0118).
