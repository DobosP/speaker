...
- Current non-real logic gate: 7,569 passing checks as 7,566 low-priority broad passes plus three exact-condition reruns (logging enabled, repository lock mode 600, and the bounded-read race isolated); 13 skipped, 24 model-only deselected, 11 warnings. Public command/noise focused gate: 235 + 1 isolated; matrix-v5 lock/materializer focused rerun 93; current APM/DTD 6.
...
- The exact public command/noise lock and no-download preparer bind 57 private schema-v4 cases from official Speech Commands v0.02 and ECCC v1.2. Typed positive/forbidden targets distinguish command recall, precision, pair/case false positives, speech negatives, and silence without committing selected per-case transcript rows, members, speakers, or paths (ADR-0117).
...
- On the 57-case command/noise gate, Faster-Whisper Small reached 18/20 clean commands but only 1/6 noisy ECCC commands and accepted 11/21 documented ECCC command confusions. Zipformer with its required one-second short-clip context reached 16/20, 1/6, and 0/21 respectively. Both remain development comparators; the Zipformer tail is not endpoint/latency evidence and no default changed (ADR-0117).
...
- Acquire and validate the four private conversation/STT corpora. Use the new public command/noise gate for hard-noise model or frontend work; do not rerun stock Moonshine threshold-zero external finalization, and require a patched reset/tail-safe native build before any internal/external causal pair. Never change runtime defaults from development evidence (ADR-0112/0113/0115/0116/0117).
