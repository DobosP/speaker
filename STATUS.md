...
Last verified: 2026-07-28 on Linux ROG: logic suite (`-m "not real_model"`) 5628 passed/13 skipped/20 deselected. `tools/diagnose_run.py`/`tools/replay_barge.py` blind-verdict fix landed: two of tonight's diagnostic tools could silently print a passing verdict instead of admitting they couldn't see. (1) The self-interrupt heartbeat regex required `clip=`/`underruns=` fields absent from pre-2026-06-18 logs, so on those bundles every heartbeat silently failed to parse, every barge event's speaking-state stayed unknown, and the documented 12-fire self-interrupt cascade in `logs/runs/run-20260608-181250.txt` (`.agents/backlog.md:168-183`) reported verdict=CLEAN; fixed both ways — the regex now also parses the old format (that log now correctly reports self-interrupt-likely, 10/12 suspects), and a new UNAVAILABLE guard forces a non-PASS verdict/exit code if a future format drift again leaves zero heartbeats parsed on a log with real playback/barge activity. (2) A silent (effectively all-zero) `.ref.wav` — 10/13 of the `logs/live` SUITE bundles are exactly this, a broken capture — used to process into meaningless-but-normal-looking zero/null output in both tools; both now detect it explicitly ("playback reference is silent -- echo attribution impossible") and mark the affected sections/exit code non-clean (`diagnose_run --wav`: UNAVAILABLE findings + `ref_wav_status`; `replay_barge`: distinct `REF-SILENT` status, exit 3), while output on a real-ref pair (`run-20260618-004500`) and a new-format log (`vault-phrases-20260716-184748`) stayed byte-identical to before the fix. 11 new/updated unit tests (7 in `tests/test_diagnose_run.py`, 4 in new `tests/test_replay_barge.py`) pin both behaviors with synthetic fixtures, no private bundles required.
Previous (2026-07-27, Linux ROG): logic suite (`-m "not real_model"`) 5617 passed/13 skipped; real-model replay lane (test_replay_recorded + test_asr_live_quality + replay_recorded_voice_test) 12 passed in 2:35. Harness crash-visibility fix landed: for ~10 days the real-model lane hard-crashed with rc 255 and zero output (config root cause: a half-finished Kokoro switch in config.local.json left tts_voices on the Kokoro family while tts_model still named the VITS file — sherpa-onnx's Kokoro loader calls C++ exit(-1) on that) and the staged runner still reported PASS because its verdict counted outcomes only. Closed all three blindness mechanisms: ReportParser/LLMSummary now turn non-zero-rc-with-zero-outcomes (and zero-outcome rc-0) runs into an explicit FAIL with crash evidence; tests/conftest.py writes an eager fsynced `tests-<run_id>.started.json` marker whose orphan (no matching summary) is flagged by the runner as a hard interpreter death; and `build_tts` gained an O(metadata) ONNX family preflight that raises a readable RuntimeError naming tts_model/tts_voices on a Kokoro/VITS mismatch instead of dying natively (fail-open when metadata is unreadable). Verified with a fake os._exit(255) test through tools/run_tests.py (reports FAIL, rc 255, orphan marker flagged) plus unit + real-model mismatch repro tests in tests/test_testing_harness.py and tests/test_tts_backend.py.
Previous (2026-07-17, Linux ROG): full suite 5568 passed/16 skipped/9 warnings; Parakeet/Small aggregate WER 0.00/6 exact; strict recorded 9; APM 6; pinned artifact check, NeMo decode probe, and selected installer dry-run green.
