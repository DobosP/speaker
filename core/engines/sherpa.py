...
_FIFO_DECAY = 0.9
_FIFO_SEC_MAX = 4.0
# A healthy PortAudio stop()+close() may take several callback periods. Keep the
...
class _KwsFeedRoute(str, Enum):
    """Exact echo-safety route under which KWS PCM was submitted."""

    IDLE = "idle"
    IN_APP_AEC = "in_app_aec"
    VERIFIED_OS_ECHO = "verified_os_echo"


@dataclass(frozen=True, slots=True)
class _KwsSpeakerAuthority:
    """Reader-time compatible/warm speaker gate generation."""

    gate: object = field(repr=False, compare=False)
    gate_authority: object = field(repr=False)
    policy: "_KwsSpeakerPolicy"
    gate_incarnation: int
    warm_generation: int


@dataclass(frozen=True, slots=True)
class _KwsSpeakerPolicy:
    """Exact effective config policy used by an ambiguous KWS receipt."""
...
@dataclass(frozen=True, slots=True)
class _KwsPcmChunk:
    """One exact submitted KWS sample interval."""
...
class _KwsPcmLedger:
    """Capture-owner continuity ledger for one fresh native KWS stream."""

    scope: Optional["_KwsControlAuthority"] = None
    chunks: deque = field(default_factory=deque)
    retained_start: int = 0
    submitted_end: int = 0
    last_capture_sequence: Optional[int] = None
    last_source_sample_end: Optional[int] = None


class _SpeakerPcmStatus(str, Enum):
    """Closed result of one explicit-PCM speaker comparison."""
...
@dataclass(frozen=True, slots=True)
class _SpeakerPcmDecision:
    """Side-effect-free decision for one caller-owned PCM region."""

    status: _SpeakerPcmStatus
    identity_pcm: Optional[np.ndarray] = field(default=None, repr=False, compare=False)
    similarity: Optional[float] = None
    threshold: Optional[float] = None
    reject_threshold: Optional[float] = None
    raw_samples: int = 0
    voiced_samples: int = 0
    authority: object = field(default=None, repr=False, compare=False)
...
class _KwsControlAuthority:
    ...
    source_domain: object = field(repr=False, compare=False)
    kws_sample_rate_hz: int
    authority_source_generation: int
    authority_source_device: object = field(repr=False, compare=False)
    os_echo_route_verified: bool
    route: _KwsFeedRoute
    route_owner: object = field(repr=False, compare=False)
    speaker_authority_available: bool
    speaker_authority: Optional[_KwsSpeakerAuthority] = field(
        repr=False,
        compare=False,
    )
    speaking: bool
...
class _WordCutStopDecision:
    ...
    def needs_speaker(self) -> bool:
        # ADR-0042's own-TTS ambiguity guard is independent of the optional
        # multi-voice filter used for generic word-cut. A novel exact control is
        # enrollment-free; a control that resembles current/recent TTS is not.
        return self.requested and self.reads_like_own_speech
...
class SherpaConfig:
    ...
    # A short exact command may end before the streaming transducer has enough
    # acoustic clock to publish its partial. On the first quiet block only, pad
    # that detector feed with zeros up to this duration (hard-capped at 0.5 s).
    # The padding is decoder timing only: it never joins candidate, speaker, or
    # normal-ASR evidence, and the capture-block reset debounce is unchanged.
    barge_word_cut_decoder_flush_sec: float = 0.4
    # Silero can stay false on OS-EC-degraded near-end speech even when sustained
    # post-EC, pre-application-gain energy is well above the device-calibrated
    # InputAGC floor. This substitution is deliberately opt-in while the private
    # virtual-delay gate validates it. On an already verified OS echo route only,
    # it may admit that energy to the lexical detector after this relative margin
    # and consecutive-block debounce. It never cuts by level: command/novel-word,
    # own-speech, and speaker-authority gates remain unchanged. No complete fresh
    # calibration -> fallback inert.
    barge_word_cut_energy_fallback_enabled: bool = False
    barge_word_cut_energy_margin_db: float = 9.0
    barge_word_cut_energy_min_blocks: int = 3
    # Optional multi-voice word-cut filter. Single-voice barge-in defaults to
    # identity-free lexical authority: a canonical control or at least four NEW
    # non-own words on a verified echo-cancel route. Set this true only when a
    # compatible enrollment should reject other speakers during playback. The
    # lexical floor remains mandatory and cannot be lowered by either mode.
    # Independently, ADR-0042 always identity-gates a STOP-class control that
    # resembles current/recent own TTS; novel exact controls remain immediate.
    ...
    # Optional Speaker-ID gates for normal finals, generic multi-voice word-cut,
    # and own-TTS-ambiguous controls. Novel exact Stop/cancel stays an open
    # fail-safe control and ordinary identity-free word-cut keeps its lexical floor.
...
class SherpaOnnxEngine(AudioEngine):
    ...
    def __init__(
        ...
        *,
        ...
        self._virtual_route_stop = threading.Event()
        self._virtual_route_thread: Optional[threading.Thread] = None
        self._virtual_route_failure = ""
        self._virtual_route_failure_lock = threading.Lock()
        ...
        # The raw primary OnlineRecognizer exists only between model build and
        # capture-session transfer. Every online operation and stream lifetime
        # after that transfer belongs to this exact-thread owner.
        ...
        self._capture_effect_condition = threading.Condition()
        self._capture_epoch = 0
        self._capture_effects = 0
        self._capture_callback_context = threading.local()
        # Set from the first stop instruction until the capture owner + every
        ...
        self._speaker_gate_state_lock = threading.Lock()
        self._speaker_gate: Optional[SpeakerGate] = None
        self._speaker_gate_incarnation: int = 0
        ...
        # 100 ms capture thread; it defers identity-gated authority until this is true.
        self._speaker_gate_warmed: bool = False
        self._speaker_gate_warm_generation: int = 0
        self._speaker_gate_warm_model_generation: Optional[int] = None
        ...
        # Consecutive post-EC blocks above the calibrated energy fallback floor.
        self._word_cut_energy_run: int = 0
        # Private virtual-harness clock edge. This is independent of the
        # fallback run: Silero-positive blocks are onset evidence but must not
        # warm fallback admission.
        self._virtual_near_end_above: bool = False
    ...
    def _build(self) -> None:
        ...
        self._replace_speaker_gate(None)
    ...
    def _bind_virtual_capture_route(self) -> None:
        ...
        if binder is None:
            return
        binder.bind_capture(timeout_sec=5.0)
        ok, detail = binder.verify(require_playback=False)
        if not ok:
            raise RuntimeError(f"virtual delay capture route is unverified: {detail}")
        log.info("[autotest-route] capture verified: %s", detail)

    def _prepare_virtual_playback_route(self) -> None:
        binder = self._virtual_audio_binder
        if binder is None:
            return
        binder.bind_playback(timeout_sec=5.0)
        ...
        if not ok:
            ...
            raise RuntimeError(f"virtual delay duplex route is unverified: {detail}")
        log.info("[autotest-route] playback target verified before start: %s", detail)

    def _authorize_virtual_playback_route(self) -> None:
        binder = self._virtual_audio_binder
        if binder is None:
            return
        # A newly proven graph starts a new authority epoch; no evidence from a
        # prior/lost graph may count toward its consecutive-block debounce.
        self._word_cut_energy_run = 0
        self._virtual_near_end_above = False
        ok, detail = binder.verify(require_playback=True)
        with self._virtual_route_failure_lock:
            ...
            if not ok or revoked:
                ...
                self._word_cut_energy_run = 0
                self._virtual_near_end_above = False
                reason = detail if not ok else "route revocation already started"
                raise RuntimeError(
                    f"virtual delay duplex route is unverified: {reason}"
                )
            wants_word_cut = bool(
            ...
            self._word_cut_route_verified = wants_word_cut
            self._os_echo_route_verified = bool(
                wants_word_cut or self.config.capture_voice_comm
            )
            # Capture can consume the silent->speaking reset while playback is
            # still being opened and authority is intentionally false. Re-arm it
            # after the atomic duplex grant so this same reply creates its
            # dedicated word-cut stream on the next capture block.
            self._barge_sustain_reset_pending = True
        log.info("[autotest-route] duplex verified: %s", detail)

    @property
    def virtual_route_failure(self) -> str:
        with self._virtual_route_failure_lock:
            return self._virtual_route_failure

    def _fail_virtual_route(self, detail: str) -> None:
        """Hard-stop private playback before publishing route-proof loss."""

        with self._virtual_route_failure_lock:
            if self._virtual_route_failure or self._virtual_route_failure_in_progress:
                return
            self._virtual_route_failure_in_progress = True
        # Fatal route loss is also a capture-stop linearization. A PortAudio read
        # already in flight may return after playback teardown clears `_speaking`;
        # invalidate its epoch and make the post-read boundary drop that block so
        # it cannot emit ordinary KWS/ASR/final callbacks after proof loss.
        ...
        with self._receipt_lock:
            ...
            with self._out_lock:
                ...
                if fifo is not None:
                    # Route loss is not a user barge: retain no de-click tail on
                    # a sink whose target is no longer trusted.
        ...
        try:
            if out is not None:
                ...
                if callable(abort):
                    ...
                    out.stop()
                out.close()
        except Exception:  # noqa: BLE001 - publish the fatal proof either way
            log.exception("virtual-route output teardown failed")
        finally:
            with self._virtual_route_failure_lock:
                self._virtual_route_failure = detail
                self._virtual_route_failure_in_progress = False

    def _fail_virtual_capture_if_unexpected(self, detail: str) -> bool:
        if (
            self._virtual_audio_binder is None
            or self._capture_stopping.is_set()
            or self.virtual_route_failure
        ):
            return False
        self._fail_virtual_route(detail)
        return True

    def _virtual_route_monitor_loop(self) -> None:
        binder = self._virtual_audio_binder
        if binder is None:
            return
        while self._running.is_set() and not self._virtual_route_stop.wait(0.1):
            require_playback = bool(binder.playback_bound)
            ok, detail = binder.verify(require_playback=require_playback)
            if ok:
                continue
            log.error("[autotest-route] route proof lost: %s", detail)
            self._fail_virtual_route(detail)
            return

    def _start_virtual_route_monitor(self) -> None:
        if self._virtual_audio_binder is None:
            return
        prior = self._virtual_route_thread
        if prior is not None and prior.is_alive():
            raise RuntimeError("previous virtual-route monitor is still active")
        self._virtual_route_stop.clear()
        self._virtual_route_thread = threading.Thread(
            target=self._virtual_route_monitor_loop,
            name="autotest-virtual-route",
            daemon=True,
        )
        self._virtual_route_thread.start()
    ...
    def _resolve_capture_domain(
        ...
        *,
        ...
        if binder is not None:
            capture_verified, detail = binder.verify(require_playback=False)
            voice_comm = (
                binder.contract.provenance if capture_verified else "unverified"
            )
            route_verified = bool(capture_verified and binder.fully_bound)
    ...
    def _start_receipt_dispatcher(self) -> None:
        with self._receipt_lock:
            # Linearize dispatcher admission with stop/startup rollback. A
            # speak_tracked() caller may have observed _running immediately
            # before the stop fence; it must not resurrect a receipt owner.
    ...
    def _stop_receipt_dispatcher(self) -> None:
        ...
        if thread is not None:
            ...
            if thread.is_alive():
                ...
                # Retain ownership. A later start() must see this live worker and
                # fail before re-setting its shared run event; dropping the
                # reference here could resurrect two receipt consumers.
                return
        with self._receipt_lock:
    ...
    def _capture_callback_is_current(self, epoch: Optional[int] = None) -> bool:
        ...
        Direct unit/helper calls have no capture context and retain historical
        behavior. Live capture installs its run epoch; queued finals carry it
    ...
    def _enter_capture_effects(self, epoch: int) -> bool:
        ...
        stop() invalidates the epoch under the same condition. Thus either this
    ...
    def _leave_capture_effects(self) -> None:
        with self._capture_effect_condition:
            self._capture_effects -= 1
    ...
    def _capture_effects_are_idle(self) -> bool:
        with self._capture_effect_condition:
            return self._capture_effects == 0

    @staticmethod
    def _thread_start_was_observed(thread: Optional[threading.Thread]) -> bool:
        """Whether a capture Thread crossed Python's start publication point."""

        if thread is None:
            return False
        missing = object()
        started = getattr(thread, "_started", missing)
        if started is not missing:
            try:
                if started.is_set():
                    return True
            except Exception:  # noqa: BLE001 - nonstandard injected Thread
                pass
            return getattr(thread, "ident", None) is not None
        # Narrow compatibility for simple injected worker doubles that expose
        # only join()/is_alive(); an alive double represents an already-started
        # owner, while a quiescent one remains conservatively unpublished.
        try:
            return bool(thread.is_alive())
        except Exception:  # noqa: BLE001 - malformed injected worker
            return getattr(thread, "ident", None) is not None

    @classmethod
    def _thread_may_start_or_is_alive(
        cls,
        thread: Optional[threading.Thread],
    ) -> bool:
        """Treat an ambiguously launched Thread as live until proven finished."""
    ...
    def _close_capture_input(self) -> bool:
        """Try one bounded physical input teardown, retaining on failure."""
        capture_stream = self._stream_in
        if capture_stream is None:
            return True
        try:
            try:
                closed = capture_stream.close(
                    teardown_timeout=_INPUT_TEARDOWN_TIMEOUT_SEC
                )
            except TypeError:
                # Narrow compatibility for simple injected test streams.
                closed = capture_stream.close()
        except Exception:  # noqa: BLE001 - device may already be gone
            log.exception("input close failed; retaining stream handle")
            return False
        if closed is False:
            return False
    ...
    def _rollback_failed_capture_processor_launch(
        ...
        *,
        ...
        A constructor failure precedes ownership transfer, but any exception
        raised while ``Thread.start`` waits for publication may arrive after the
        OS thread began. Fence first, then join-or-retain both capture owners
        ...
        grace = max(0.01, float(self.config.block_sec) + 0.05)
        capture_thread = self._capture_thread
        if (
            capture_start_ambiguous
            and capture_thread is not None
            and not self._thread_start_was_observed(capture_thread)
        ):
            # CPython launches the OS thread before the child publishes ident
            # and sets _started. A Python signal-handler exception can land in
            # the parent's _started.wait() during that gap. Give publication
            # one bounded opportunity; if it still has not happened, retain the
            # Thread and every resource as a potentially pending owner.
            started = getattr(capture_thread, "_started", None)
            wait = getattr(started, "wait", None)
            if callable(wait):
                wait(timeout=grace)
        if capture_thread is not None and (
            (
                not capture_start_ambiguous
                and not self._thread_start_was_observed(capture_thread)
            )
            or (
                self._thread_start_was_observed(capture_thread)
                and not capture_thread.is_alive()
            )
        ):
            # Constructor/pre-transfer failures leave an unstarted Thread.
            # A published thread that already finished is equally safe to
            # normalize. Do this before a dropped-receipt callback can re-enter
            # stop(); an ambiguous unpublished Thread stays retained.
        ...
        # A terminal callback may re-enter stop() and enqueue its playback
        # sentinel while no playback worker exists. Remove that callback-owned
        # residue before a future cleanup/restart can observe it.
        with self._receipt_lock:
            self._drain_play_q()
            self._terminalize_unbound_receipts()
        ...
        if lane is not None:
            lane.close()
            self._capture_control_lane = None
        ...
        if (
            ...
            capture_thread.join(timeout=grace)
        if media_session is not None:
            media_session.join(timeout=grace)
        capture_processor_active = self._thread_may_start_or_is_alive(
            capture_thread
        )
        ...
        if (
            ...
            and capture_stream is not None
        ):
            abort_read = getattr(capture_stream, "abort_read", None)
            if callable(abort_read):
                try:
                    abort_read(timeout=grace)
            ...
            if capture_reader_alive and media_session is not None:
        ...
        if (
            ...
            except Exception:  # noqa: BLE001 - retain the live owner fail-closed
                log.exception(
                    "streaming decode session cleanup failed after capture "
                    "launch failure"
                )
    ...
    def start(self, callbacks: EngineCallbacks) -> None:
        ...
        if self._stream_in is not None:
            raise RuntimeError(
                "cannot start audio: previous input stream was retained; "
                "restart the process after the native capture operation exits"
            )
        ...
        log.info(
            ...
            or (
                self._streaming_decode_session is not None
                and not self._streaming_decode_session.closed
            ),
        ...
        self._stream_in = _RecoveringInputStream(
            ...
            pre_start_validate=(
                (lambda _candidate, _attempt: self._bind_virtual_capture_route())
                if self._virtual_audio_binder is not None
                else None
            ),
            recovery_enabled=self._virtual_audio_binder is None,
        )
        try:
    ...
    def stop(self) -> None:
        ...
        self._word_cut_energy_run = 0
        self._virtual_near_end_above = False
        retained_capture_resources_on_entry = self._capture_resource_hold.is_set()
        self._capture_resource_hold.set()
        with self._capture_effect_condition:
            # Epoch invalidation is the linearization point: no later capture
            # side-effect lease can be admitted for this run. Existing leases
        ...
        # configured block and the capture loop exits before processing it.  Join
        # that owner before physically handing PortAudio's buffer back.  Only a
        ...
        capture_processor_alive = self._thread_may_start_or_is_alive(
            capture_thread
        )
        capture_reader_alive = bool(
            media_session is not None
        ...
        with self._receipt_lock:
            self._drain_play_q()
            if (
                not self._capture_resource_hold.is_set()
                and not retained_capture_resources_on_entry
            ):
                # Claim the shared playback objects before closing them.  The
                # playback worker may reach its finally block while stop() is
                # still joining capture; clearing the shared identity makes
                # exactly one side own native teardown and leaves restart state
                # free of stale closed handles.
                with self._out_lock:
                    fifo = self._fifo
                    out = self._out_stream
                    self._fifo = None
        ...
        if wake_play_thread:
            ...
            try:
                ...
                # Admissions are gated above; a full queue can only contain
                # work that raced the gate before it closed. Drop it with
                # receipts, then reserve the wake slot without blocking.
                self._drain_play_q()
                try:
                    self._play_q.put_nowait(sentinel)
                except queue.Full:
        ...
        if self._capture_resource_hold.is_set() or retained_capture_resources_on_entry:
            if not self._release_retained_audio_resources_if_idle(
                ...
                log.error(
                    ...
                    "worker joins; retaining shared audio resources"
                )
    ...
    def stop_speaking(self) -> None:
        ...
        if cut:
            self._emit_capture_callback(self._cb.on_metric, BARGE_IN_STOP)
    ...
    def _replace_speaker_gate(self, gate: Optional[SpeakerGate]) -> None:
        """Install one gate incarnation and retire every prior warm receipt."""

        lock = getattr(self, "_speaker_gate_state_lock", None)
        if lock is None:  # narrow ``__new__`` fixtures
            lock = threading.Lock()
            self._speaker_gate_state_lock = lock
        with lock:
            self._speaker_gate = gate
            self._speaker_gate_incarnation = (
                int(getattr(self, "_speaker_gate_incarnation", 0)) + 1
            )
            self._speaker_gate_warm_generation = (
                int(getattr(self, "_speaker_gate_warm_generation", 0)) + 1
            )
            self._speaker_gate_warmed = False
            self._speaker_gate_warm_model_generation = None

    def _publish_speaker_gate_warm(
        self,
        gate: SpeakerGate,
        incarnation: int,
        warmed: bool,
        ...
        """Publish only if the model warmed is still the installed gate."""

        if (
            type(incarnation) is not int
            or type(warmed) is not bool
            or (model_generation is not None and type(model_generation) is not int)
            or (warmed and model_generation is None)
        ):
            return False
        lock = getattr(self, "_speaker_gate_state_lock", None)
        if lock is None:
        ...
        with lock:
            if (
                self._speaker_gate is not gate
                or self._speaker_gate_incarnation != incarnation
            ):
                return False
            if self._speaker_gate_warmed is not warmed:
                self._speaker_gate_warm_generation += 1
            self._speaker_gate_warmed = warmed
            self._speaker_gate_warm_model_generation = (
                model_generation if warmed else None
            )
            return warmed
    ...
    def _warm_speaker_gate(self) -> bool:
        ...
        with lock:
            gate = self._speaker_gate
            incarnation = int(getattr(self, "_speaker_gate_incarnation", 0))
            if gate is None:
                return False
            warmed = bool(self._speaker_gate_warmed)
            warm_model_generation = getattr(
                self,
                "_speaker_gate_warm_model_generation",
                None,
            )
        if warmed:
            try:
                current_authority = SpeakerGate.authority_state(gate)
            except Exception:  # noqa: BLE001 - retry the real warm below
                current_authority = None
            if (
                type(current_authority) is SpeakerGateAuthority
                and current_authority.model_generation == warm_model_generation
            ):
                with lock:
                    if (
                        self._speaker_gate is gate
                        and self._speaker_gate_incarnation == incarnation
                        and self._speaker_gate_warmed
                        and self._speaker_gate_warm_model_generation
                        == warm_model_generation
                    ):
                        return True
            self._publish_speaker_gate_warm(
                gate,
                incarnation,
                False,
                None,
        ...
        try:
            ...
            warmed = bool(
                ...
                and type(before_warm) is SpeakerGateAuthority
                and type(warm_authority) is SpeakerGateAuthority
                and before_warm.gate_token is warm_authority.gate_token
                and before_warm.model_generation == warm_authority.model_generation
            )
            published = self._publish_speaker_gate_warm(
                gate,
                incarnation,
                warmed,
                (
                    warm_authority.model_generation
                    if type(warm_authority) is SpeakerGateAuthority
                    else None
                ),
            )
            if published:
    ...
    # --- ASR text helpers ---
    def _claim_streaming_decode_session(
        self,
    ) -> Optional[SherpaStreamingDecodeSession]:
        """Transfer the primary online recognizer into one run-scoped owner.

        Production calls this immediately before capture-thread launch. Direct
        finite-loop tests may call it from the capture thread itself. A closed
        prior-run session is metadata only and can be replaced; an unclosed one
        must never be silently aliased into a second run.
        """

        session = self._streaming_decode_session
        if session is not None and not session.closed:
            if self._recognizer is not None:
                raise RuntimeError(
                    "online recognizer and decode session cannot both be live"
                )
            return session
    ...
    def _new_asr_stream(
        ...
        *,
        ...
        owner = recognizer
        if owner is None:
            owner = self._streaming_decode_session or self._recognizer
        if owner is None:
            raise RuntimeError("no online recognizer is available")
    ...
    def _finalize_and_dispatch(
        ...
        if not final_text.strip():
            ...
            if verification is OwnerVerification.VERIFIED and not owner_lineage_intact:
                ...
                self._emit_capture_callback(
                    self._cb.on_metric,
                    "speaker_mixed_lineage_unverified",
                    capture_epoch=capture_epoch,
                )
    ...
    def _snapshot_capture_context(self, stamp: CaptureStamp) -> CaptureBlockContext:
        ...
        speaker_authority_scope = (
            self._snapshot_kws_speaker_authority()
            if authority_matches and speaker_authority
            else None
        )
        if isinstance(gate, SpeakerGate):
            speaker_authority = speaker_authority_scope is not None
        return CaptureBlockContext(
            ...
            speaker_authority=speaker_authority_scope,
            speaking=speaking,
    ...
    @staticmethod
    def _kws_speaker_policy_is_valid(policy: object) -> bool:
        if type(policy) is not _KwsSpeakerPolicy:
            return False
        return bool(
            type(policy.sample_rate) is int
            and policy.sample_rate > 0
            and type(policy.min_sec) is float
            and math.isfinite(policy.min_sec)
            and policy.min_sec >= 0.10
            and type(policy.window_sec) is float
            and math.isfinite(policy.window_sec)
            and policy.window_sec > 0.0
            and type(policy.accept_threshold) is float
            and math.isfinite(policy.accept_threshold)
            and 0.0 <= policy.accept_threshold <= 1.0
            and type(policy.reject_threshold) is float
            and math.isfinite(policy.reject_threshold)
            and 0.0 <= policy.reject_threshold <= policy.accept_threshold
        )

    def _snapshot_kws_speaker_policy(
        self,
        gate_authority: SpeakerGateAuthority,
    ) -> Optional[_KwsSpeakerPolicy]:
        """Freeze the unchanged effective word-cut STOP speaker policy."""

        sample_rate = getattr(self.config, "sample_rate", None)
        gate_input = getattr(self.config, "speaker_gate_input", True)
        if (
            isinstance(sample_rate, bool)
            or not isinstance(sample_rate, int)
            or sample_rate <= 0
            or type(gate_input) is not bool
            or type(gate_authority) is not SpeakerGateAuthority
            or type(gate_authority.threshold) is not float
            or not math.isfinite(gate_authority.threshold)
            or not 0.0 <= gate_authority.threshold <= 1.0
        ):
            return None
        raw_min_sec = getattr(
            self.config,
        ...
        raw_block_sec = self.config.block_sec
        raw_window = getattr(
            self.config,
        ...
        raw_threshold = getattr(
            ...
            gate_authority.threshold,
        )
        raw_reject_threshold = getattr(
            self.config,
        ...
        if any(
            ...
            for value in (
                ...
                raw_window,
                raw_threshold,
                raw_reject_threshold,
            )
        ):
            return None
        try:
            configured_min_sec = float(raw_min_sec)
            block_sec = float(raw_block_sec)
            configured_window = float(raw_window)
            raw_threshold_float = float(raw_threshold)
            raw_reject_threshold_float = float(raw_reject_threshold)
            raw_values = (
                configured_min_sec,
                block_sec,
                configured_window,
                raw_threshold_float,
                raw_reject_threshold_float,
            )
            if any(not math.isfinite(value) for value in raw_values):
                return None
            min_sec = max(0.10, configured_min_sec)
            configured_threshold = max(
                0.0,
                min(1.0, raw_threshold_float),
            )
            accept_threshold = (
                max(gate_authority.threshold, configured_threshold)
                if gate_input
        ...
        if (
            min_sec < 0.0
            or block_sec <= 0.0
            or configured_window <= 0.0
            or not 0.0 <= accept_threshold <= 1.0
            or not 0.0 <= reject_threshold <= accept_threshold
        ):
            return None
        policy = _KwsSpeakerPolicy(
            sample_rate=sample_rate,
            min_sec=min_sec,
            window_sec=max(block_sec, configured_window),
            accept_threshold=accept_threshold,
            reject_threshold=reject_threshold,
        )
        return policy if self._kws_speaker_policy_is_valid(policy) else None

    def _snapshot_kws_speaker_authority(
        self,
    ) -> Optional[_KwsSpeakerAuthority]:
        """Capture one compatible, enrolled, warmed gate generation."""
        ...
        with lock:
            ...
            gate_incarnation = getattr(self, "_speaker_gate_incarnation", None)
            warm_generation = getattr(
                self,
                "_speaker_gate_warm_generation",
                None,
            )
            warm_model_generation = getattr(
                self,
            ...
            warmed = getattr(self, "_speaker_gate_warmed", False)
        if not isinstance(gate, SpeakerGate):
            return None
        try:
            if (
                type(gate_incarnation) is not int
                or type(warm_generation) is not int
                or type(warm_model_generation) is not int
                or type(warmed) is not bool
                or not warmed
            ):
                return None
            gate_authority = SpeakerGate.authority_state(gate)
            if type(gate_authority) is not SpeakerGateAuthority:
                return None
            policy = self._snapshot_kws_speaker_policy(gate_authority)
            if policy is None:
                return None
            with lock:
                state_current = bool(
                    self._speaker_gate is gate
                    and self._speaker_gate_incarnation == gate_incarnation
                    and self._speaker_gate_warm_generation == warm_generation
                    and self._speaker_gate_warm_model_generation
                    == warm_model_generation
                    and self._speaker_gate_warmed
                )
            if (
                not state_current
                or not gate_authority.enrolled
                or gate_authority.model_generation != warm_model_generation
                or not SpeakerGate.authority_is_current(gate, gate_authority)
            ):
                return None
        except Exception:  # noqa: BLE001 - reader authority fails closed
            return None
        return _KwsSpeakerAuthority(
            gate=gate,
            gate_authority=gate_authority,
            policy=policy,
            gate_incarnation=gate_incarnation,
            warm_generation=warm_generation,
        )

    def _kws_speaker_authority_is_current(
        self,
        authority: Optional[_KwsSpeakerAuthority],
    ) -> bool:
        """Recheck the exact gate/model/enrollment/policy/warm generation."""

        if authority is None:
            return self._snapshot_kws_speaker_authority() is None
        if type(authority) is not _KwsSpeakerAuthority:
            return False
        lock = getattr(self, "_speaker_gate_state_lock", None)
        if lock is None:
            return False
        with lock:
            ...
            warmed = getattr(self, "_speaker_gate_warmed", False)
        if gate is not authority.gate or not isinstance(gate, SpeakerGate):
            return False
        if (
            type(authority.gate_incarnation) is not int
            or type(authority.warm_generation) is not int
            or type(authority.gate_authority) is not SpeakerGateAuthority
            or not self._kws_speaker_policy_is_valid(authority.policy)
            or type(gate_incarnation) is not int
            or type(warm_generation) is not int
            or type(warm_model_generation) is not int
            or type(warmed) is not bool
            or authority.gate_incarnation != gate_incarnation
            or authority.warm_generation != warm_generation
            or authority.gate_authority.model_generation != warm_model_generation
            or not warmed
        ):
            return False
        try:
            return bool(
                SpeakerGate.authority_is_current(
                    gate,
                    authority.gate_authority,
                )
                and self._snapshot_kws_speaker_policy(authority.gate_authority)
                == authority.policy
            )
        except Exception:  # noqa: BLE001 - authority recheck fails closed
            return False
    ...
    def _kws_control_authority(
        ...
        *,
        ...
        return _KwsControlAuthority(
            ...
            source_sample_rate_hz=captured.sample_rate_hz,
            source_sample_count=int(captured.pcm.size),
            ...
            kws_sample_rate_hz=self.config.sample_rate,
            ...
            speaker_authority_available=context.speaker_authority_available,
            speaker_authority=context.speaker_authority,
    ...
    def _kws_control_authority_is_current(
        ...
        if (
            ...
            or type(authority.capture_sequence) is not int
            or type(authority.source_generation) is not int
            or type(authority.source_sample_rate_hz) is not int
            or type(authority.source_sample_count) is not int
            or type(authority.source_sample_start) is not int
            or type(authority.source_sample_end) is not int
            or type(authority.kws_sample_rate_hz) is not int
            or type(authority.authority_source_generation) is not int
            or type(authority.os_echo_route_verified) is not bool
            or type(authority.route) is not _KwsFeedRoute
            or type(authority.speaker_authority_available) is not bool
            ...
            return False
        if authority.speaker_authority_available is not (
            authority.speaker_authority is not None
        ):
            return False
        if (
            authority.capture_sequence <= 0
            or authority.source_sample_rate_hz <= 0
            or authority.source_sample_count <= 0
            or authority.kws_sample_rate_hz <= 0
            or authority.source_sample_start < 0
            or authority.source_sample_end <= authority.source_sample_start
            or authority.source_sample_end - authority.source_sample_start
            != authority.source_sample_count
        ):
            return False
        if type(self._capture_epoch) is not int:
            return False
        if (
            type(getattr(self.config, "sample_rate", None)) is not int
            or self.config.sample_rate != authority.kws_sample_rate_hz
        ):
            return False
        ...
        try:
            ...
            source_domain_matches = bool(
                getattr(self, "_capture_resolution", None) == authority.source_domain
            )
        ...
        if not (
            source_matches
            and source_domain_matches
            and captured_authority_matches
            and live_authority_matches
        ):
            return False
        if (
            ...
            return False
        if authority.route is _KwsFeedRoute.IDLE:
            if authority.speaking or authority.route_owner is not None:
                return False
        elif authority.route is _KwsFeedRoute.IN_APP_AEC:
            if (
                not authority.speaking
                or authority.route_owner is None
                or self._aec is not authority.route_owner
            ):
                return False
        elif authority.route is _KwsFeedRoute.VERIFIED_OS_ECHO:
            if (
                not authority.speaking
                or authority.route_owner is not None
                or self._aec is not None
                or not authority.os_echo_route_verified
            ):
                return False
        else:
            return False
        if not self._kws_speaker_authority_is_current(authority.speaker_authority):
            return False
        with self._gen_lock:
            if (
    ...
    def _claim_playback_kws_if_current(
        ...
        *,
        ...
        # Installing the claim first closes playback-start admission. Recheck
        # every source/route/speaker generation after that barrier before the
    ...
    @staticmethod
    def _kws_authority_has_same_scope(
        left: _KwsControlAuthority,
        right: _KwsControlAuthority,
    ) -> bool:
        """Compare every continuity field except per-block coordinates."""

        if (
            type(left) is not _KwsControlAuthority
            or type(right) is not _KwsControlAuthority
        ):
            return False
        try:
            devices_match = bool(
                left.source_device == right.source_device
                and left.authority_source_device == right.authority_source_device
            )
            domains_match = bool(left.source_domain == right.source_domain)
        except Exception:  # noqa: BLE001 - opaque identity fails closed
            return False
        left_speaker = left.speaker_authority
        right_speaker = right.speaker_authority
        if left_speaker is None or right_speaker is None:
            speakers_match = left_speaker is right_speaker
        else:
            speakers_match = bool(
                type(left_speaker) is _KwsSpeakerAuthority
                and type(right_speaker) is _KwsSpeakerAuthority
                and SherpaOnnxEngine._kws_speaker_policy_is_valid(left_speaker.policy)
                and SherpaOnnxEngine._kws_speaker_policy_is_valid(right_speaker.policy)
                and left_speaker.gate is right_speaker.gate
                and left_speaker.gate_authority == right_speaker.gate_authority
                and left_speaker.policy == right_speaker.policy
                and left_speaker.gate_incarnation == right_speaker.gate_incarnation
                and left_speaker.warm_generation == right_speaker.warm_generation
            )
        return bool(
            devices_match
            and domains_match
            and speakers_match
            and left.capture_epoch == right.capture_epoch
            and left.capture_generation == right.capture_generation
            and left.source_generation == right.source_generation
            and left.source_sample_rate_hz == right.source_sample_rate_hz
            and left.kws_sample_rate_hz == right.kws_sample_rate_hz
            and left.authority_source_generation == right.authority_source_generation
            and left.os_echo_route_verified is right.os_echo_route_verified
            and left.route is right.route
            and left.route_owner is right.route_owner
            and left.speaker_authority_available is right.speaker_authority_available
            and left.speaking is right.speaking
            and left.speak_generation == right.speak_generation
            and left.playback_generation == right.playback_generation
        )

    def _kws_pcm_window_sample_limit(
        self,
        authority: Optional[_KwsControlAuthority] = None,
    ) -> Optional[int]:
        """Return the unchanged word-cut speaker window in model samples."""

        speaker_authority = (
            authority.speaker_authority
            if type(authority) is _KwsControlAuthority
            else None
        )
        if type(
            speaker_authority
        ) is _KwsSpeakerAuthority and self._kws_speaker_policy_is_valid(
            speaker_authority.policy
        ):
            sample_rate = speaker_authority.policy.sample_rate
            block_sec = speaker_authority.policy.window_sec
            window_sec = speaker_authority.policy.window_sec
        else:
            sample_rate = getattr(self.config, "sample_rate", None)
            block_sec = getattr(self.config, "block_sec", None)
            window_sec = getattr(
                self.config,
                "barge_word_cut_speaker_window_sec",
                2.0,
        ...
        if (
            ...
            or not isinstance(sample_rate, int)
            or sample_rate <= 0
            or isinstance(block_sec, bool)
            or isinstance(window_sec, bool)
        ):
            return None
        try:
            seconds = max(float(block_sec), float(window_sec))
        except (TypeError, ValueError, OverflowError):
            return None
        if not math.isfinite(seconds) or seconds <= 0.0:
            return None
        samples = _checked_ceil_size(float(sample_rate) * seconds)
        return samples if samples is not None and samples > 0 else None

    def _ensure_kws_pcm_ledger(self) -> _KwsPcmLedger:
        ledger = getattr(self, "_kws_pcm_ledger", None)
        if type(ledger) is not _KwsPcmLedger:
            ledger = _KwsPcmLedger()
            self._kws_pcm_ledger = ledger
        return ledger

    def _retire_keyword_stream(self) -> None:
        """Capture-owner terminal retirement for stop/finally/rebuild."""

        self._kws_pcm_ledger = _KwsPcmLedger()
        self._kws_continuity_dirty = False
        kws = getattr(self, "_kws", None)
        old_stream = getattr(self, "_kws_stream", None)
        self._kws_stream = None
        if kws is not None and old_stream is not None:
            try:
                kws.reset_stream(old_stream)
            except Exception:  # noqa: BLE001 - retirement remains authoritative
                pass
    ...
    def _mark_keyword_continuity_dirty(self) -> None:
        """Retire PCM now and defer native rotation until an eligible feed."""

        self._kws_pcm_ledger = _KwsPcmLedger()
        self._kws_continuity_dirty = True

    def _retire_keyword_continuity_if_active(self) -> None:
        """Rotate once when an ineligible block interrupts KWS continuity."""

        ledger = self._ensure_kws_pcm_ledger()
        if (
            ledger.scope is not None
            or ledger.submitted_end != 0
            or ledger.chunks
            or ledger.last_capture_sequence is not None
            or ledger.last_source_sample_end is not None
        ):
            self._mark_keyword_continuity_dirty()

    def _prepare_kws_submission(
        self,
        authority: _KwsControlAuthority,
    ) -> bool:
        """Bind one exact adjacent reader block to a fresh native stream."""

        if type(authority) is not _KwsControlAuthority:
            self._retire_keyword_continuity_if_active()
            return False
        if bool(getattr(self, "_kws_continuity_dirty", False)):
            self._reset_keyword_stream_after_discontinuity()
        ledger = self._ensure_kws_pcm_ledger()
        same_scope = bool(
            ledger.scope is not None
            and self._kws_authority_has_same_scope(ledger.scope, authority)
        )
        adjacent = bool(
            same_scope
            and ledger.last_capture_sequence is not None
            and ledger.last_source_sample_end is not None
            and authority.capture_sequence == ledger.last_capture_sequence + 1
            and authority.source_sample_start == ledger.last_source_sample_end
        )
        if ledger.scope is not None and not adjacent:
            self._reset_keyword_stream_after_discontinuity()
            ledger = self._ensure_kws_pcm_ledger()
        if self._kws_stream is None:
            return False
        if ledger.scope is None:
            ledger.scope = authority
            ledger.retained_start = 0
            ledger.submitted_end = 0
        return True

    def _append_kws_submitted_pcm(
        self,
        pcm: np.ndarray,
        authority: _KwsControlAuthority,
    ) -> None:
        """Record the exact owned buffer successfully submitted to native KWS."""

        ledger = self._ensure_kws_pcm_ledger()
        if ledger.scope is None or not self._kws_authority_has_same_scope(
            ledger.scope,
            authority,
        ):
            raise ValueError("KWS PCM scope changed during native submission")
        count = int(pcm.size)
        if count <= 0 or ledger.submitted_end > sys.maxsize - count:
    ...
    def _copy_kws_pcm_interval(self, start: int, end: int) -> Optional[np.ndarray]:
        """Copy one exact hole-free interval from the submitted-sample ledger."""
        ...
        for chunk in ledger.chunks:
            ...
            if chunk.start > cursor:
                return None
            take_end = min(end, chunk.end)
            pieces.append(chunk.pcm[cursor - chunk.start : take_end - chunk.start])
            cursor = take_end
            if cursor == end:
                break
    ...
    def _kws_word_pcm_regions(
        self,
        binding: KwsPhraseBinding,
        timestamps,
    ) -> Optional[tuple[np.ndarray, ...]]:
        """Resolve strict pinned decoder frames to one exact PCM region per word."""

        if (
            type(binding) is not KwsPhraseBinding
            or type(timestamps) is not list
            or len(timestamps) != len(binding.tokens)
            or not timestamps
        ):
            return None
        ledger = self._ensure_kws_pcm_ledger()
        sample_rate = (
            ledger.scope.kws_sample_rate_hz
            if type(ledger.scope) is _KwsControlAuthority
            else None
        )
        if (
            isinstance(sample_rate, bool)
            or not isinstance(sample_rate, int)
            or sample_rate <= 0
            or (sample_rate * _KWS_TOKEN_FRAME_MS) % 1000
        ):
            return None
        frame_samples = (sample_rate * _KWS_TOKEN_FRAME_MS) // 1000
        half_sample_sec = 0.5 / float(sample_rate)
        frame_sec = _KWS_TOKEN_FRAME_MS / 1000.0
        frames: list[int] = []
        previous = -1
        for timestamp in timestamps:
            if type(timestamp) is not float or not math.isfinite(timestamp):
                return None
            frame_float = timestamp / frame_sec
            if (
                not math.isfinite(frame_float)
                or frame_float < 0.0
                or frame_float > sys.maxsize // frame_samples
            ):
                return None
            ...
            if (
                frame < 0
                or frame <= previous
                # sherpa-onnx exposes a Python float but its timestamp may have
                # traversed float32 storage.  Accept only the pinned 40 ms grid,
                # with at most half one model sample of representation error.
                or abs(timestamp - frame * frame_sec) > half_sample_sec
                or frame > sys.maxsize // frame_samples
            ):
                return None
            frames.append(frame)
            previous = frame
        token_samples = [frame * frame_samples for frame in frames]
        if any(
            sample < ledger.retained_start or sample >= ledger.submitted_end
            for sample in token_samples
        ):
            return None
        counts = binding.word_token_counts
        if (
            type(counts) is not tuple
            or not 0 < len(counts) <= 2
        ...
        for word_index, token_count in enumerate(counts):
            ...
            last_token = token_offset + token_count - 1
            if word_index + 1 < len(counts):
                word_end = token_samples[token_offset + token_count]
            else:
                last_frame = frames[last_token]
                if last_frame >= sys.maxsize // frame_samples:
                    return None
                word_end = (last_frame + 1) * frame_samples
            if (
                word_start < ledger.retained_start
                or word_end > ledger.submitted_end
    ...
    def _capture_loop(
        ...
        *,
        ...
        try:
            ...
            if self._running.is_set():
                log.exception(
                    "capture loop initialization crashed -- the assistant has "
                    "stopped listening"
                )
            self._running.clear()
        finally:
            ...
            if (
                ...
                try:
                    decode_session.close()
                except Exception:  # noqa: BLE001 - ownership failure is fatal
                    log.exception("streaming decode session did not close cleanly")
                    self._running.clear()
            media_session = self._capture_media_session
            if media_session is not None:
                media_session.request_stop()
        ...
        *,
        ...
        try:
            while self._running.is_set():
                ...
                kws_route = None
                kws_route_owner = None
                if not speaking_for_kws:
                    kws_route = _KwsFeedRoute.IDLE
                elif self._aec is not None:
                    kws_route = _KwsFeedRoute.IN_APP_AEC
                    kws_route_owner = self._aec
                elif context_authority_current and block_context.os_echo_route_verified:
                    kws_route = _KwsFeedRoute.VERIFIED_OS_ECHO
                kws_authority = (
                    self._kws_control_authority(
                        captured,
                        block_context,
                        route=kws_route,
                        route_owner=kws_route_owner,
                    )
                    if kws_route is not None
                    else None
                )
                kws_authority_was_current = bool(
                    kws_authority is not None
                    and self._kws_control_authority_is_current(kws_authority)
                )
                command_consumed = False
                kws_participated = False
                if kws_authority is not None:
                ...
                if (
                    ...
                    # makes this queued PCM stale for word-cut/normal ASR.  It
                    # was already submitted to KWS, so retire that prefix too:
                    # an ABA back to the same visible scope must not let the
                    # stale block bridge into a later keyword.
                    self._mark_keyword_continuity_dirty()
                    continue
            ...
            # The native stream and its exact submitted-sample ledger have one
            # capture owner and one lifetime. No stopped/restarted run may
            # inherit either half, including re-entrant stop retention.
            self._retire_keyword_stream()
            if effects_admitted:
                self._leave_capture_effects()
            if captured_in_flight is not None and callable(media_finish):
    ...
    def _poll_keywords(
        ...
        *,
        ...
        """
        kws = self._kws
        if kws is None or self._kws_stream is None:
            return False
        if (
        ...
        try:
            source_pcm = np.asarray(samples)
            if source_pcm.ndim != 1:
                raise ValueError("KWS PCM must be one-dimensional")
            sample_count = int(source_pcm.size)
            ...
            if owned_pcm.size != sample_count or not bool(
                np.all(np.isfinite(owned_pcm))
            ):
                raise ValueError("KWS PCM must be finite")
        except _CaptureBlockRejected:
        ...
        if not allow_publish:
            # A guarded/stale block is a real hole in the submitted clock. It is
            # skipped entirely and forces a new native+ledger time origin even
            # when the preceding gap already left an empty fresh stream.
            self._mark_keyword_continuity_dirty()
            return False
        if control_authority is not None:
            if not self._kws_control_authority_is_current(control_authority):
                self._mark_keyword_continuity_dirty()
                return False
            if not self._prepare_kws_submission(control_authority):
                return False
        else:
            self._retire_keyword_continuity_if_active()
        # Scope/adjacency preparation may have replaced the native stream.
        ks = self._kws_stream
        if ks is None:
            return False
        keyword = ""
        ...
        try:
            ...
            ks.accept_waveform(feed_sample_rate, owned_pcm)
            if control_authority is not None:
                self._append_kws_submitted_pcm(owned_pcm, control_authority)
            ...
            if keyword:
                normalized_hit = (
                    keyword if playback_scoped else normalize_command(keyword)
                )
                if (
                    playback_scoped
                    and control_authority is not None
                    and normalized_hit in {"stop", "wait"}
                ):
                    resolved_binding = resolve_kws_stop_binding(
                        getattr(self, "_kws_phrase_bindings", ()),
                        result_label=normalized_hit,
                    ...
                    if resolved_binding is not None:
                        ambiguity_surfaces = tuple(
                            dict.fromkeys(
                                (
                                    resolved_binding.surface,
                                    *collapsed_label_surfaces(normalized_hit),
                                )
                            )
                        )
                        own_tts_ambiguous = bool(
                            ambiguity_surfaces
                            and any(
                                self._reads_like_own_speech(text)
                                for text in ambiguity_surfaces
                            )
                        )
                        if (
                            own_tts_ambiguous
                            and control_authority.speaker_authority_available
                            and control_authority.speaker_authority is not None
                        ):
                            timestamp_reader = getattr(kws, "timestamps", None)
                            if not callable(timestamp_reader):
                                raise TypeError("KWS token timestamps are unavailable")
                            speaker_regions = self._kws_word_pcm_regions(
                                resolved_binding,
                                timestamp_reader(ks),
                            )
                self._reset_keyword_stream_after_discontinuity()
            ...
            return False
        if not keyword:
            # This block was already accepted by native KWS.  If its exact
            # authority was lost while decode ran, retire the prefix here;
            # callers must not be required to notice the transient before a
            # later apparently-equal scope resumes feeding.
            if (
                control_authority is not None
                and not self._kws_control_authority_is_current(control_authority)
            ):
                self._mark_keyword_continuity_dirty()
            return False
        ...
        if playback_scoped:
            ...
            # alias reads like current/recent TTS. The exact phrase PCM is
            # speaker-scored below; word-cut keeps its independent evidence.
            ambiguity_surfaces = collapsed_label_surfaces(normalized_label)
            if resolved_binding is not None:
                ambiguity_surfaces = tuple(
                    dict.fromkeys((resolved_binding.surface, *ambiguity_surfaces))
                )
            if not ambiguity_surfaces:
                return False
            if own_tts_ambiguous or any(
                self._reads_like_own_speech(text) for text in ambiguity_surfaces
            ):
                speaker_authority = (
                    control_authority.speaker_authority
                    if control_authority is not None
                    else None
                )
                if (
                ...
                if (
                    ...
                    or control_authority is None
                    or not self._kws_control_authority_is_current(control_authority)
                ):
                    self._kws_own_echo_suppressions += 1
                    log.info(
                        "KWS hit %r suppressed: own-TTS ambiguity lacked "
                        "current per-word speaker authority",
                        keyword,
                    )
                    return False
    ...
    def _playback_loop(self) -> None:
        ...
        try:
            while self._running.is_set():
                ...
                try:
                    if out is None:
                        ...
                        self._prepare_virtual_playback_route()
                        with self._out_lock:
                        ...
                        if self._virtual_audio_binder is not None:
                            detail = (
                                "virtual playback stream became inactive; "
            ...
            if not self._capture_resource_hold.is_set():
                with self._out_lock:
                    # stop() may already have atomically claimed this exact
                    # stream.  Only the owner still published in shared state
                    # may close it here, otherwise both threads can race a
                    # native stop/close on the same PortAudio handle.
                    claimed_out = out if self._out_stream is out else None
                ...
                # only if the capture/playback owner genuinely remains active
                # after its bounded joins.
                log.debug(
                    "deferring output stream teardown while capture resources are held"
                )
    ...
    def _prepare_explicit_speaker_pcm(
        self,
        pcm,
        *,
        min_sec: float,
        ...
        """Prepare caller-owned PCM with the unchanged word-cut voiced rule."""

        sample_rate = (
            getattr(self.config, "sample_rate", None)
        ...
        if (
            ...
            or not math.isfinite(block_sec)
            or not math.isfinite(configured_window_sec)
        ):
            return _SpeakerPcmDecision(_SpeakerPcmStatus.ERROR)
        required_sec = max(0.0, required_sec)
        min_samples = max(0, int(sample_rate * required_sec))
        raw_samples = int(candidate.size)
        if raw_samples <= 0 or (min_samples > 0 and raw_samples < min_samples):
            return _SpeakerPcmDecision(
        ...
        if framed:
            ...
            if peak > 1e-8:
                voiced_samples = (
                    int(np.count_nonzero(energies >= max(1e-5, peak * 0.15))) * frame
                )
    ...
    def _score_explicit_speaker_pcm_batch(
        ...
        *,
        min_sec: float,
        window_sec: Optional[float] = None,
        expected_authority: Optional[_KwsSpeakerAuthority] = None,
    ) -> tuple[_SpeakerPcmDecision, ...]:
        """Score one/two explicit regions without caller-state mutation or wait."""
        ...
        if expected_authority is not None:
            ...
            min_sec = expected_authority.policy.min_sec
            window_sec = expected_authority.policy.window_sec
            score_sample_rate = expected_authority.policy.sample_rate
        else:
            score_sample_rate = getattr(self.config, "sample_rate", None)
        gate = getattr(self, "_speaker_gate", None)
        if gate is None:
            return tuple(
                _SpeakerPcmDecision(_SpeakerPcmStatus.UNAVAILABLE) for _ in pcm_regions
        ...
        try:
            ...
            if expected_authority is not None:
                ...
                if type(receipt) is not SpeakerSimilarityBatchReceipt:
                    raise TypeError("speaker batch receipt has an invalid type")
                similarities = receipt.similarities
                gate_threshold = receipt.authority.threshold
                if (
                    type(receipt.authority) is not SpeakerGateAuthority
                    or type(similarities) is not tuple
                    or len(similarities) != len(prepared)
                    or type(gate_threshold) is not float
                    or not math.isfinite(gate_threshold)
                    or not 0.0 <= gate_threshold <= 1.0
                    or not SpeakerGate.authority_is_current(
                        gate,
                        receipt.authority,
                    )
                ):
                ...
                if (
                    receipt.authority != expected_authority.gate_authority
                    or not self._kws_speaker_authority_is_current(expected_authority)
                ):
                    return tuple(
                        replace(item, status=_SpeakerPcmStatus.STALE)
                ...
                if len(prepared) != 1:
                    return tuple(
                        replace(item, status=_SpeakerPcmStatus.UNAVAILABLE)
                        for item in prepared
                    )
            ...
            if expected_authority is not None:
                ...
                threshold = (
                    max(gate_threshold, configured_threshold)
            ...
            if (
                ...
                raise ValueError("speaker policy threshold is invalid")
            decisions: list[_SpeakerPcmDecision] = []
            for item, raw_similarity in zip(prepared, similarities, strict=True):
                if type(raw_similarity) is not float:
                    raise TypeError("speaker similarity must be an exact float")
                similarity = raw_similarity
    ...
    def _word_cut_speaker_decision(
        ...
        *,
        ...
        fails closed for generic talk-over when the optional filter is enabled.
        ``identity_required`` preserves ADR-0042 independently: a STOP-class
        control resembling current/recent TTS must obtain speaker authority even
        when generic word-cut is enrollment-free. Novel canonical controls remain
        immediate.
        """
        if not identity_required and not bool(
        ...
        if gate is None or not gate.is_enrolled:
            ...
            if not st.get("_speaker_unavailable_warned"):
                ...
                log.warning(
                    "word-cut speaker authority unavailable; identity-gated cut deferred"
                )
            return "defer"
        required_sec = (
            float(min_sec)
        ...
        if not self._speaker_gate_warmed:
            ...
            if not st.get("_speaker_cold_warned"):
                ...
                log.warning(
                    "word-cut speaker authority not warmed; identity-gated cut deferred"
                )
            return "defer"
        ...
        try:
            ...
            log.warning(
                "word-cut speaker decision failed; identity-gated cut deferred",
                exc_info=True,
            )
            return "defer"
        decision = self._score_explicit_speaker_pcm_batch(
            (candidate,),
            min_sec=required_sec,
        )[0]
        if decision.status in (
            _SpeakerPcmStatus.INSUFFICIENT,
            _SpeakerPcmStatus.COLD,
            _SpeakerPcmStatus.UNAVAILABLE,
            _SpeakerPcmStatus.BUSY,
        ):
            st["speaker_deferred"] = st.get("speaker_deferred", 0) + 1
            if decision.status is _SpeakerPcmStatus.BUSY:
                st["speaker_busy_deferred"] = st.get("speaker_busy_deferred", 0) + 1
            return "defer"
        if decision.status is _SpeakerPcmStatus.ERROR:
            st["_speaker_error"] = True
            st["speaker_errors"] = st.get("speaker_errors", 0) + 1
            log.warning("word-cut speaker decision failed; identity-gated cut deferred")
            return "defer"
        if decision.status is _SpeakerPcmStatus.STALE:
            return "defer"
        similarity = decision.similarity
        threshold = decision.threshold
        reject_threshold = decision.reject_threshold
        identity = decision.identity_pcm
        if (
            similarity is None
            or threshold is None
            or reject_threshold is None
            or identity is None
        ):
            st["_speaker_error"] = True
            st["speaker_errors"] = st.get("speaker_errors", 0) + 1
        ...
        accepted = decision.status is _SpeakerPcmStatus.ACCEPT
        if accepted:
            ...
            if trimmed:
                st["pcm_trimmed_samples"] = st.get("pcm_trimmed_samples", 0) + trimmed
        ambiguous = decision.status is _SpeakerPcmStatus.DEFER
        if ambiguous:
            st["speaker_deferred"] = st.get("speaker_deferred", 0) + 1
    ...
    def _word_cut_authority_min_words(self) -> int:
        ...
        Lexical evidence must first prove that the playback recognizer heard
        something command-like rather than an empty or short garbled echo burst.
        Local floor values from zero through three therefore may not reopen the
        known 2-3-word echo failure, with or without optional speaker filtering.
    ...
    def _word_cut_energy_fallback_voiced(
        self,
        energy_samples,
        *,
        allow_admission: bool = True,
    ) -> bool:
        """Whether sustained calibrated near-end energy may supplement VAD.

        This is only an *ingest* decision for the lexical word-cut recognizer.
        It never fires a cut by level and is inert unless capture is on the
        already attested OS echo route and this capture epoch owns a complete,
        stable pre-gain calibration. ``energy_samples`` is in that same post-OS-
        EC, pre-InputAGC/pre-denoiser domain; processed ASR PCM is never compared
        against the pre-gain floor.
        """
        cal = getattr(self, "_last_calibration", None)
        agc = getattr(self, "_input_agc", None)
        try:
            enabled = bool(
                getattr(
                    self.config,
                    "barge_word_cut_energy_fallback_enabled",
                    False,
                )
            )
            margin_db = float(
                getattr(self.config, "barge_word_cut_energy_margin_db", 0.0)
            )
            block_sec = float(self.config.block_sec)
            calibrate_sec = float(self.config.input_calibrate_sec)
        ...
        if (
            not enabled
            or margin_db <= 0.0
            or not math.isfinite(margin_db)
            or agc is None
            or not bool(getattr(self.config, "input_calibrate", False))
            or not isinstance(cal, dict)
            or calibrate_sec <= 0.0
            or block_sec <= 0.0
            or not math.isfinite(calibrate_sec)
            or not math.isfinite(block_sec)
            or floor <= 0.0
            ...
            or energy_samples is None
            or not bool(getattr(self, "_word_cut_route_verified", False))
        ):
            self._word_cut_energy_run = 0
            self._virtual_near_end_above = False
            return False

        expected_blocks = max(1, int(round(calibrate_sec / block_sec)))
        if calibrated_blocks < expected_blocks:
            self._word_cut_energy_run = 0
            self._virtual_near_end_above = False
            return False

        level = float(rms(energy_samples))
        # Bound a malformed local override before exponentiation; an absurdly
        # high margin should make the fallback unreachable, never crash capture.
        threshold = floor * (10.0 ** (min(120.0, margin_db) / 20.0))
        above = bool(math.isfinite(level) and level >= threshold)
        if self._virtual_audio_binder is not None:
            if above and not self._virtual_near_end_above:
                log.info("[autotest-route] near-end onset")
            self._virtual_near_end_above = above
        if not allow_admission:
            # Silero owns ingest for this block. The independent marker above is
            # still the harness capture-arrival clock, but neither debounce nor
            # fallback telemetry may warm on VAD-positive PCM.
            self._word_cut_energy_run = 0
            return False
        if above:
            self._word_cut_energy_run += 1
            st = self._wc_stats
            st["energy_above_floor"] = st.get("energy_above_floor", 0) + 1
            st["energy_run_max"] = max(
                st.get("energy_run_max", 0), self._word_cut_energy_run
            )
        else:
            self._word_cut_energy_run = 0
            return False
        ...
        if self._word_cut_energy_run < minimum:
            return False
        st = self._wc_stats
        st["energy_fallback_blocks"] = st.get("energy_fallback_blocks", 0) + 1
        if self._word_cut_energy_run == minimum:
            st["energy_fallback_starts"] = st.get("energy_fallback_starts", 0) + 1
        return True

    def _reset_word_cut_energy_epoch(self) -> None:
        """Drop fallback debounce and the private capture-onset edge latch."""
    ...
    def _barge_word_cut_step(
        ...
        *,
        ...
        # forbidden from warming a future fallback run.
        energy_voiced = self._word_cut_energy_fallback_voiced(
            energy_samples,
            allow_admission=not vad_voiced,
        )
        # Near-end evidence uses the same pre-gain domain as calibration. This is
        # the only valid comparison for distinguishing "voice never survived the
        # canceller" from "voice arrived but VAD never accepted it as speech".
        self._wc_window_update(energy_samples, now, vad_voiced=vad_voiced)
        ...
        # wiped the very words a talk-over had accumulated. Once a burst has fed
        # voiced PCM, its first one or two genuinely quiet blocks are fed only as
        # a bounded decoder flush. They never join candidate/speaker evidence;
        # the existing third-quiet reset remains the hard burst boundary.
        decode_blocks = None
        if not (vad_voiced or energy_voiced):
            ...
            # fallback's consecutive-block admission merely because an earlier
            # burst left a recognizer stream alive. Only truly below-threshold
            # quiet may flush already-attested voiced content.
            if not self._word_cut_fed_stream or self._word_cut_energy_run:
                return False
            # This block was appended to the VAD lookback before the decision.
            # It may advance the decoder, but must never ride into a later
            # speaker/candidate window if speech resumes before the reset.
            self._clear_word_cut_vad_preroll()
            if self._word_cut_candidate_samples <= 0:
                return False
            flush_limit = min(2, max(0, debounce - 1))
            if self._word_cut_quiet_run > flush_limit:
                return False
            flush = np.asarray(samples, dtype="float32").reshape(-1)
            real_flush_samples = int(flush.size)
            if self._word_cut_quiet_run == 1:
                try:
                    flush_sec = float(
                        getattr(
                            self.config,
                            "barge_word_cut_decoder_flush_sec",
                            0.4,
                        )
                    )
                except (TypeError, ValueError, OverflowError):
                    flush_sec = 0.0
                if math.isfinite(flush_sec) and flush_sec > 0.0:
                    target = min(
                        int(self.config.sample_rate * 0.5),
                        max(
                            flush.size,
                            int(self.config.sample_rate * flush_sec),
                        ),
                    )
                    if target > flush.size:
                        flush = np.concatenate(
                            (
                                flush,
                                np.zeros(target - flush.size, dtype="float32"),
                            )
                        )
            decode_blocks = [flush]
            st["quiet_flush_blocks"] = st.get("quiet_flush_blocks", 0) + 1
            st["quiet_flush_samples"] = st.get("quiet_flush_samples", 0) + int(
                flush.size
            )
            st["quiet_flush_padding_samples"] = st.get(
                "quiet_flush_padding_samples", 0
            ) + max(0, int(flush.size) - real_flush_samples)
        else:
            self._word_cut_quiet_run = 0
        ...
        # the callback; a loss that starts later observes an already-valid cut.
        route_guard = (
        ...
        with route_guard:
            route_valid = bool(self._word_cut_route_verified)
            if self._virtual_audio_binder is not None:
                route_valid = bool(
                    route_valid
                    and not self._virtual_route_failure
                    and not self._virtual_route_failure_in_progress
                )
            if not route_valid:
                try:
                    recognizer.reset(stream)
                ...
                self._clear_word_cut_candidate()
                self._clear_word_cut_vad_preroll()
                st["route_revoked"] = st.get("route_revoked", 0) + 1
                return False

            st["cuts"] = st.get("cuts", 0) + 1
            # Seed the CLEAN normal stream from candidate USER PCM before firing.
            # The dedicated detection stream may contain a folded own-echo prefix
            # and is discarded by the handoff.
            self._promote_word_cut_candidate(
    ...
    def _wc_window_update(
        self, energy_samples, now: float, *, vad_voiced: Optional[bool] = None
    ) -> None:
        """Accumulate the ~2s near-end evidence window while word-cut is live and
        emit one "word-cut near-end:" INFO line per window: post-OS-EC, pre-gain
        mic RMS (avg + peak), the VAD-active fraction, and its calibrated floor.
        ...
        if (
            ...
            else bool(self._vad is not None and self._vad.is_speech_detected())
        ):
            w["vad"] += 1
        if now - w["start"] < 2.0:
            return
    ...
    def _emit_word_cut_funnel(self) -> None:
        ...
        log.info(
            ...
            "energy_fallback=%d energy_starts=%d energy_run_max=%d "
            "resets=%d dropped_words=%d "
