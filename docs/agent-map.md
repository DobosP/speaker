...
| EdAcc endpoint-integrity diagnostic | `tools/edacc_endpoint_integrity_eval.py`, `tools/capture_replay_eval.py`, `core/endpointing.py`, `core/engines/_semantic_hold.py`, `always_on_agent/logical_turn_shadow.py`, ADR-0147/0148/0153/0155 | exact retained corpus/profile/model binding; acoustic, HOLD-only Smart Turn, and opt-in native reset-and-accumulate through the production capture seam; optional matched raw-composition aggregates plus exact inline raw-commit → selected-final/typed-abort lineage under separate strict schemas; aggregate privacy/no-clobber and replay-v2 lifecycle tests; async worker delivery, dispatcher, supervisor, authority, and live evidence remain separate gates |
