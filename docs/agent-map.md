...
| EdAcc endpoint-integrity diagnostic | `tools/edacc_endpoint_integrity_eval.py`, `tools/capture_replay_eval.py`, `core/endpointing.py`, ADR-0147 | exact retained corpus/profile/model binding, acoustic versus HOLD-only Smart Turn through the production capture seam, aggregate privacy/no-clobber tests, then a separately opt-in reset experiment; never infer a live default |
