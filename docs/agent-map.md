...
| EdAcc endpoint-integrity diagnostic | `tools/edacc_endpoint_integrity_eval.py`, `tools/capture_replay_eval.py`, `core/endpointing.py`, `core/engines/_semantic_hold.py`, ADR-0147/0148 | exact retained corpus/profile/model binding; acoustic, HOLD-only Smart Turn, and opt-in native reset-and-accumulate through the production capture seam; aggregate privacy/no-clobber and replay-v2 lifecycle tests; never infer a live default without owner `./live.sh` A/B |
