...
| Configured final-command FileReplay gate | `tools/production_final_stt_eval.py`, `tools/streaming_stt/final_metrics.py`, `core/engines/file_replay.py`, ADR-0118 | verify exact corpus/config/model/source/runtime/provider binding and aggregate-only decision metrics headlessly; the selected view lacks live VAD-owned timing/recovery, so keep it separate from owner recordings and live bare-speaker validation |
| Public conversation/STT fixture lock | `tools/public_conversation_fixture.py`, the four `tools/prepare_*_conversation_fixture.py` materializers, `tools/streaming_stt/public-conversation-96.lock.json`, `tools/public_voice_eval_matrix.py`, ADR-0113 | focused raw-source/lock/receipt/tamper tests first; prepare four private 24-case corpora outside Git, use the fixture entry point to validate/run/revalidate the existing sequential suite, then keep model and live A/B as separate gates |
