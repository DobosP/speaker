...
| Echo-probe lifecycle/result, runtime AEC-delay provenance, and serialized open-speaker guidance | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_echo_probe.py tests/test_interrupt_suite.py tests/test_aec_seam.py tests/test_denoise.py -q -rs` | `177 passed, 1 skipped` in 2.08s: fakes cover exact one-stop ownership for every invoked start; post-start setup/warm-up/body/tail faults; `KeyboardInterrupt` cleanup and preservation; post-start/stop `SystemExit` rc1 including code zero; primary/secondary stop-fault precedence; error-only failure with no normal report or runtime snapshot; strict one-object JSON emission and invalid-serialization fallback; scalar snapshot/reset provenance; active-AEC retained-owner `snapshot_unavailable`; unchanged interrupt-suite child consumption; and affected serialized coherence/DTD/uncoupled/top-level guidance that is descriptive only, omits headphone and low-level core-session suggestions, and names only `./live.sh` followed by the explicit retained-run `python -m tools.live_audio_ab logs/runs/run-<id>.txt` check for physical acceptance. The adjacent AEC/capture/teardown command uses the same prefix with `tests/test_sherpa_media_session.py tests/test_sherpa_streaming_decode_session.py tests/test_sherpa_playback.py tests/test_input_calibration.py tests/test_aec_seam.py -q -rs` and passes `214 passed, 1 skipped`; the skip in both commands is the absent DTLN ONNX. APM/DTD separately passes `6 passed` in 0.68s. Scoped Ruff lint passes for the tool and both tests, both test files pass Ruff format, the whole-file echo-probe format check remains red on pre-existing debt, changed guidance/test regions are formatted, and whitespace is green. A normally returned bounded stop is not proof that native cleanup completed. No audible-stimulus, model, microphone, audio device, open-speaker, GPU, network, runtime/config/default/backend, delay/ERLE quality, physical-suite, or live evidence follows (ADR-0175/0177/0178). |
| Interrupt-suite outcome contract (headless) | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_interrupt_suite.py -q` | `63 passed`: fakes cover the closed `diagnostic_outcome` object; exit 0 on any valid coupled zero cell, exit 1 on valid coupled cells with none zero, and exit 2 without valid coupled evidence; unique lexical all-candidate reporting without a winner; malformed/explicit-error row counting plus the terminal publication increment; valid uncoupled rows not incrementing errors; `matrix_partial == (error_cells > 0)` without expected-cell inference; empty/unknown microphone preflight; JSON persistence; publication failure forcing rc2 with cleared candidates, incremented errors, and partial true while descriptive counts may remain; forbidden legacy recommendation text; and the backend-neutral `configured-aec` label. The adjacent command uses the same prefix/flags with `tests/test_interrupt_suite.py tests/test_echo_probe.py tests/test_barge_self_interrupt.py tests/test_autotest_verdicts.py tests/test_diagnose_run.py -q` and passes `271` in 1.30s; APM/DTD separately passes `6` in 0.68s. No model, network, microphone, audio device, open-speaker, GPU, physical suite, runtime/config/default/backend, or live evidence follows (ADR-0176/0178). |
