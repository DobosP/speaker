...
| Model archive extraction (headless) | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 15 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_setup_model_archive_extraction.py tests/test_setup_doctor.py tests/test_setup_kokoro.py tests/test_setup_kws.py -q` | `230 passed`: shared punctuation/SenseVoice/Parakeet/Kokoro/KWS flatten/strip selection, bounds, exact-size atomic publication, archive special filtering, destination symlink/reparse-point/hardlink rejection or replacement, Windows path-alias rejection, rollback, existing setup contracts, and no-network local archives pass; no model download/load, GPU, audio device, runtime/default, or live evidence (ADR-0165) |
