...
| EdAcc endpoint-integrity contracts | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 15 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_edacc_endpoint_integrity_eval.py tests/test_capture_replay_eval.py -q` | exact retained-corpus/profile/Smart-Turn admission, acoustic-versus-HOLD-only capture-loop execution, fresh row policy, aggregate reduction/privacy, rechecks, and no-clobber publication pass with fakes; no model, audio device, reset strategy, runtime default, or live claim follows (ADR-0147) |
