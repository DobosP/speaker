...
| Playback-time KWS phrase/generation authority (headless) | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_kws_contract.py tests/test_setup_kws.py tests/test_virtual_audio_engine.py tests/test_sherpa_media_session.py tests/test_sherpa_playback.py tests/test_barge_confirm.py tests/test_semantic_hold_capture.py tests/test_core_runtime.py -q` | `349 passed in 4.63s`. Fakes prove the code-pinned exact ordered shipped six-row phone-token setup binding, setup lexicon-drift and bounded/read-stable parser rejection, raw-exact native token plus closed-label resolution before reset, all-alias own-TTS ambiguity abstention, `stop`/`wait` canonicalization through typed `on_control_stop_result` without the mutable command map, missing-callback pre-native abstention, unchanged idle generic KWS, stale capture-epoch/media/source/authority-source/route/speaking/speak/playback rejection, strict authority types, mutually exclusive exact claims, atomic claim/start ordering, post-claim playback-admission rejection without false own-TTS provenance, idle response deferral, callback release, admitted-only acoustic terminalization, and runtime typed STOP validation. The same prefix/flags with `tests/test_sherpa_media_session.py tests/test_semantic_hold_capture.py tests/test_setup_kws.py tests/test_barge_word_cut.py -q` passes `218` in 2.30s; the complete local-Sherpa command passes `449` in 4.52s; APM/DTD passes `6` in 1.00s. Scoped Ruff lint is green on the five implementation and six changed test files; the new contract/test files are fully Ruff-format clean, changed regions in inherited-debt files are clean, and AST, `git diff --check`, and exact 100-line STATUS checks pass. Independent adversarial review is GO with `42` focused tests in 1.59s. Preserve ADR-0082's KWS ambiguity abstention and speaker-aware word-cut fallback. No raw-KWS, speaker acceptance, model, threshold, boost, default, microphone, audio-device, open-speaker, latency, quality, or live evidence follows (ADR-0182). |
