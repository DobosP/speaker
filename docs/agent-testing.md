...
| Bounded shipped SearXNG ingestion (headless) | `/usr/bin/time -p env SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 ionice -c 3 nice -n 19 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_websearch.py -q` | `136 passed in 0.59s` (`real 0.77s`, `user 0.53s`, `sys 0.03s`). In-process HTTPX transports/raw streams, fake clocks/Events, custom backends, hostile objects, and local corpus fixtures prove exact config types/defaults/clamps; direct `httpx==0.28.1`; identity/context-managed raw streaming and close; strict headers/body/UTF-8/JSON/payload/hits; result/field/output caps; cooperative deadlines/cancel; truthful egress/detail-free errors; and legacy custom compatibility. The adjacent command appends `tests/test_capability_context_isolation.py tests/test_react_planner.py` and passes `186 in 0.84s` (`real 1.01s`, `user 0.71s`, `sys 0.05s`). With only paths replaced, `tests/test_sensitivity.py tests/test_llm_egress_policy.py` passes `108 in 0.57s` (`real 0.75s`, `user 0.61s`, `sys 0.04s`); imports pass `315 in 2.02s` (`real 2.31s`, `user 2.16s`, `sys 0.13s`); and APM/DTD passes `6 in 0.81s` (`real 1.02s`, `user 0.88s`, `sys 0.13s`). Config JSON, AST2, exact HTTPX pin, scoped Ruff, diff-check, core format, five inherited/base-identical test formatter hunks with zero task-line overlap, STATUS100, frozen hashes, exact ten-path inventory, and independent rebased code/security audit are green. Phase/total bounds cannot preempt synchronous factory/enter/read/exit or JSON parse; body cap excludes transport/header/one-chunk/copy/parser-graph memory; whole JSON is parsed before prefix normalization; caps count code points and permit bounded escaped lone surrogates; exact Event/custom/direct/endpoint/proxy/same-process trust remains. No network, SearXNG, provider/model, GPU, audio, microphone, device, or live path ran (ADR-0191). |
