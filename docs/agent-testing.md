...
| Playback-time KWS word-scoped speaker authority (headless) | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_kws_contract.py tests/test_speaker_gate.py tests/test_setup_kws.py tests/test_virtual_audio_engine.py tests/test_sherpa_media_session.py tests/test_barge_word_cut.py tests/test_sherpa_playback.py tests/test_barge_confirm.py tests/test_semantic_hold_capture.py tests/test_core_runtime.py -q` | `607 passed in 6.07s`; this exact focused command covers all seven changed behavioral test files plus setup/semantic/runtime consumers. Fakes carry ADR-0182's exact six-row token/label and STOP-only fences, prove speaker-unavailable zero-PCM/no-timestamp clock continuity, and enable full PCM/generation binding only from an available compatible enrolled/warmed receipt. The config-derived committed default retains at most 32,000 float32 samples/128,000 bytes, twenty normal 100 ms chunks; no claim extends that numeric cap to arbitrary larger finite config. All KWS restarts across speaker-authority transitions while stable idle/novel label/effect eligibility runs no inference. Gap/fault/mismatch/guarded-block continuity, strict v1.13.3 40 ms timestamps, exact word spans, per-word 0.10-second voiced/current `ACCEPT`, and fail-closed states pass. The path acquires the shared inference slot once without blocking, then runs at most two synchronous native embedding calls. An entered call has no Python wall-time preemption and can delay processor shutdown; the reader advances only until bounded mailbox gaps, while stack-local clips and retained PCM stay bounded by their configured caps. The exact adjacent command `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_media_session.py tests/test_sherpa_streaming_decode_owner.py tests/test_sherpa_streaming_decode_session.py tests/test_speaker_input_gate.py tests/test_setup_doctor.py -q` passed `280` with two SWIG deprecation warnings in 4.81s. The exact complete local-Sherpa row below passed `516` in 5.30s; APM/DTD passed `6` in 1.15s. Full Ruff lint is green on three non-Sherpa production files plus seven changed tests, Sherpa F821/F822/F823 is green, 11-file AST/whitespace/exact-100-line STATUS are green, and the independent 32-hunk changed-region audit is green; whole-file format remains red on the same seven filenames as `main`. Frozen production/Sherpa SHA-256 is `8b611dabc6a42712f4e112281558ad344702d9066db117cc7386ccae3e1cec06`/`b4300779d6016016eff89b27a13a00b3a81d5acb4c6c387c674f529bcc80784d`. This proves word-level headless logic only—not phone diarization, aggregate speaker provenance, model accuracy, device/open-speaker behavior, latency, quality, or live evidence; paired owner bare-speaker A/B remains pending (ADR-0183). |
| KWS ledger object/callback PCM hardening (headless) | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_virtual_audio_engine.py -q` | `156 passed in 1.58s`. Covers exact 256/+1 chunk behavior with sample-window trim first, oldest-whole-chunk trim without native reset, current-chunk survival, cap-prefix timestamp abstention and retained-tail mapping, unavailable zero PCM, forged-state fresh recovery/no callback, `sys.maxsize - 1` exact acceptance followed by next-sample reset/abstention and later fresh-zero-origin recovery, one owned read-only phrase root with no source/ledger alias, defensive geometry rejection before allocation, independent voiced identity PCM, and weak-reference release of the old native stream/feed/regions/root/decisions before typed STOP callback entry. The exact ten-file focused gate passes 613 in 5.87s; adjacent owner/activation passes 280 with two SWIG warnings in 3.40s; complete-Sherpa passes 522 in 5.57s; and APM/DTD passes 6 in 1.78s. Sherpa F821/F822/F823, full test Ruff lint/format, AST2, whitespace, and STATUS100 are green. Whole-Sherpa lint retains the exact 23 `main` findings; formatter debt improves from 34 to 33 hunks with zero changed-line overlap. Frozen Sherpa/test/production-diff SHA-256 is `d56aaddb4c0285b3c82a01b6eb3a0ba13ee553efe8b1df305c6e2f1b426dd934`/`fdd3b3f0cfc459deb1233e7942115eb8ff6dc8bf2f5bde1b2ac64ffa1d3017e4`/`74cd781ccaca50bd94798658d22c95325b9eb09ad0bba4536911d8ea8928649a`. This is Python/headless resource evidence only: hostile finite configuration bytes, native allocator/model-call lifetime, blocked inference shutdown/backpressure, microphone/device/open-speaker behavior, and paired owner live A/B remain separate. No authority, config/default, threshold, model, label, or effect changes (ADR-0184). |
...
| Complete local Sherpa decode owner | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_sherpa_streaming_decode_owner.py tests/test_sherpa_streaming_decode_session.py tests/test_media_session.py tests/test_sherpa_media_session.py tests/test_asr_postprocess.py tests/test_barge_confirm.py tests/test_barge_word_cut.py tests/test_sherpa_vad_final_gate.py tests/test_virtual_audio_engine.py tests/test_sherpa_duplex_runtime.py -q` | `516 passed in 5.30s`: complete exact-thread DSP/VAD/primary+word-cut state machine, immutable bounded mailbox ownership, effect-lease races, scope/continuity/incarnation rotation, KWS zero-PCM versus receipt-enabled ledgers, speaker-authority transition restart, strict token/word mapping, bounded KWS/confirm returned work, code-pinned playback KWS claims/admission ordering, native-fault recovery, startup rollback, re-entrant stop/recorder cleanup, restart, fail-closed active-hotword incompatibility, and KWS-terminal/confirm cleanup pass with fakes. Speaker embedding calls remain synchronous/non-preemptible after the one nonblocking slot acquisition. No model, microphone, audio-device, open-speaker, latency, quality, or live evidence follows (ADR-0110/0111/0114/0171/0172/0183) |
