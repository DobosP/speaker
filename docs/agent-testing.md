...
| Duck-confirm/KWS terminal cleanup | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_barge_confirm.py tests/test_virtual_audio_engine.py tests/test_barge_word_cut.py tests/test_core_runtime.py tests/test_setup_kws.py -q` | `240 passed`: raw-vs-processed confirm-source characterization, KWS-terminal precedence/duck cleanup, established STOP/word-cut/runtime authority, and KWS setup pass with fakes. No raw-KWS stream/effect, model, threshold, microphone, audio device, open-speaker, latency, quality, default, or authority evidence follows; the complete local Sherpa gate passes `327` and APM/DTD passes `6` (ADR-0171). |
