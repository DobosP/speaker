...
| Echo-probe lifecycle/result, AEC-delay provenance, guidance, submitted-text identity, and neutral fourth stimulus | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_echo_probe.py tests/test_interrupt_suite.py tests/test_aec_seam.py tests/test_denoise.py -q -rs` | `190 passed, 1 skipped` in 2.06s: the skip is the absent DTLN ONNX at `tests/test_aec_seam.py:930`; fakes cover exact one-stop ownership; post-start setup/warm-up/body/tail faults; interrupt and stop-fault precedence; error-only failure; strict one-object JSON emission; scalar delay provenance; active-AEC retained-owner `snapshot_unavailable`; descriptive-only guidance and the exact `./live.sh` plus retained-run analyzer gate; the byte-identical first three sentences and exact fourth `This final sentence completes the quiet playback portion of the diagnostic.`; one frozen `max(1, --sentences)` cyclic plan shared by returned `engine.speak` calls and the domain-separated, unsigned-eight-byte-length-framed `stimulus_id`; the unchanged `N=3` digest `27c744fbb14bbcb74f9256a2a4db2af25c6e7ad6c70e5ea224c3c8708a06a3c6`; the new `N=4` digest `4366d6a869714ed767964bda1dc682b5264c109d5d3f12260f6e07276c502704`; rejection of pooling with the prior `N=4` digest `be277009edac56b0473730c1e5326a1b765976386aca9f26c500eab9f00822eb`; success-only identity publication; identity-free errors; raw child preservation without validation; and unchanged suite reduction. The adjacent AEC/capture/teardown command uses the same prefix with `tests/test_sherpa_media_session.py tests/test_sherpa_streaming_decode_session.py tests/test_sherpa_playback.py tests/test_input_calibration.py tests/test_aec_seam.py -q -rs`; it passes `214, 1 skipped` for the same absent DTLN ONNX in 3.84s; APM/DTD passes `6` in 0.67s. Scoped `/home/dobo/.local/bin/ruff` lint is green for the tool and both tests, both test files pass Ruff format, and AST, whitespace, and the 100-line STATUS check are green. `tests/test_interrupt_suite.py` is unchanged. Whole-tool Ruff format remains red only for inherited debt; the changed sentence region is clean. A normally returned bounded stop is not proof of native cleanup; the self-attested ID binds submitted text rather than synthesis, completed audio, or hostile-child authenticity. Missing or unequal IDs are non-poolable, while equality is only necessary. The implementation change is limited to the fourth submitted text, its synthesized stimulus and likely duration, and its derived `N=4` identity; playback control flow, wait/deadline/sleep policy, report schema and metric definitions, classification logic, runtime control path, config/default/backend boundaries, and authority remain unchanged. Observed echo, ERLE, coupling, and interruption values may differ, so old and new `N=4` results remain non-poolable. No model, microphone, audio device, open-speaker, GPU, network, physical-suite, or live evidence follows (ADR-0175/0177/0180). |
| Interrupt-suite outcome contract (headless) | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_interrupt_suite.py -q` | `64 passed` in 0.14s: fakes cover the closed `diagnostic_outcome` object; exact status/exit consistency; unique lexical all-candidate reporting without a winner; malformed/error accounting; liveness and publication failures; forbidden legacy recommendation text; the backend-neutral `configured-aec` label; and preservation of the additive raw `stimulus_id` while summary, coupling, classification, and exit logic neither validate nor consume it. The default `N=3` text and identity remain unchanged because the suite does not submit the neutralized fourth sentence. The adjacent command uses the same prefix/flags with `tests/test_interrupt_suite.py tests/test_echo_probe.py tests/test_barge_self_interrupt.py tests/test_autotest_verdicts.py tests/test_diagnose_run.py -q`; it passes `284` in 1.36s; APM/DTD passes `6` in 0.67s. `tests/test_interrupt_suite.py` is unchanged by ADR-0180. No model, network, microphone, audio device, open-speaker, GPU, physical suite, runtime-control-path/config/default/backend, identity authenticity, authority, or live evidence follows (ADR-0176/0180). |
...
| Complete local Sherpa decode owner | `SPEAKER_TEST_LOG=0 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 ionice -c 3 nice -n 15 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_sherpa_streaming_decode_owner.py tests/test_sherpa_streaming_decode_session.py tests/test_media_session.py tests/test_sherpa_media_session.py tests/test_asr_postprocess.py tests/test_barge_confirm.py tests/test_barge_word_cut.py tests/test_sherpa_vad_final_gate.py tests/test_virtual_audio_engine.py tests/test_sherpa_duplex_runtime.py -q` | `402 passed`: complete exact-thread DSP/VAD/primary+word-cut state machine, immutable mailbox ownership, effect-lease races, scope/continuity/incarnation rotation, bounded KWS/confirm native work, native-fault recovery, startup rollback, re-entrant stop/recorder cleanup, restart, fail-closed active-hotword incompatibility, and KWS-terminal/confirm cleanup pass with fakes; no model or audio-device evidence (ADR-0110/0111/0114/0171/0172) |
