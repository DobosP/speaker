...
| Isolated streaming-STT harness | `/home/dobo/work/speaker/.venv/bin/python -m pytest tests/test_streaming_stt_manifest.py tests/test_streaming_stt_protocol.py tests/test_streaming_stt_supervisor.py tests/test_streaming_stt_eval.py tests/test_streaming_stt_environment.py -q` | fake-only private source-bundle, bounded manifest/corpus/JSONL, original-process-group, aggregate/privacy, and dependency-isolation contracts pass; offline flags are not a network sandbox and resources remain worker-reported (ADR-0089) |
