...
| Atomic final-STT physical A/B | `./live.sh --run-label owner-stt-sense-unenrolled-control --llm echo --final-stt-profile sense-voice --no-speaker-enrollment`, then the same owner plan through `./live.sh --run-label owner-stt-parakeet-fws-unenrolled --llm echo --final-stt-profile parakeet-faster-whisper --no-speaker-enrollment` | one session-scoped complete profile and identity-off policy reach both readiness and the recorded core child; private summary binds the profile name/digest plus `speaker_identity_policy=off_for_session`. Use a controlled room because any audible speaker can converse/use read or confirmed low-risk tools. Physical owner review, equal plan execution, and separate bundle comparison are required; this row claims no unrun live result (ADR-0100/0138/0144/0152) |
