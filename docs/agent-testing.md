...
| Echo-probe lifecycle/result, AEC-delay provenance, guidance, and submitted-text identity | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_echo_probe.py tests/test_interrupt_suite.py tests/test_aec_seam.py tests/test_denoise.py -q -rs` | `190 passed, 1 skipped` in 2.07s: the skip is the absent DTLN ONNX at `tests/test_aec_seam.py:930`; fakes cover exact one-stop ownership; post-start setup/warm-up/body/tail faults; interrupt and stop-fault precedence; error-only failure; strict one-object JSON emission; scalar delay provenance; active-AEC retained-owner `snapshot_unavailable`; descriptive-only guidance and the exact `./live.sh` plus retained-run analyzer gate; the byte-identical four-sentence tuple; one frozen `max(1, --sentences)` cyclic plan shared by returned `engine.speak` calls and the domain-separated, unsigned-eight-byte-length-framed `stimulus_id`; exact current defaults (`N=3` digest `27c744fbb14bbcb74f9256a2a4db2af25c6e7ad6c70e5ea224c3c8708a06a3c6`, `N=4` digest `be277009edac56b0473730c1e5326a1b765976386aca9f26c500eab9f00822eb`); success-only identity publication; identity-free errors; raw child preservation without validation; and unchanged suite reduction. The adjacent AEC/capture/teardown command uses the same prefix with `tests/test_sherpa_media_session.py tests/test_sherpa_streaming_decode_session.py tests/test_sherpa_playback.py tests/test_input_calibration.py tests/test_aec_seam.py -q -rs` and passes `214, 1 skipped` for the same absent DTLN ONNX in 3.83s; APM/DTD passes `6` in 0.69s. Scoped Ruff lint is green on the tool and both tests, both tests pass Ruff format, and whitespace and AST checks are green. The only newly introduced blank-line formatter hunk was corrected, so changed helper/report/test regions are formatted while inherited whole-tool Ruff-format debt remains. A normally returned bounded stop is not proof of native cleanup; the self-attested ID binds submitted text rather than synthesis, completed audio, or hostile-child authenticity. Missing or unequal IDs are non-poolable, while equality is only necessary. No sentence byte, playback behavior, model, microphone, audio device, open-speaker, GPU, network, runtime/config/default/backend, delay/ERLE quality, physical-suite, or live evidence changes (ADR-0175/0177/0179). |
| Interrupt-suite outcome contract (headless) | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_interrupt_suite.py -q` | `64 passed` in 0.14s: fakes cover the closed `diagnostic_outcome` object; exact status/exit consistency; unique lexical all-candidate reporting without a winner; malformed/error accounting; liveness and publication failures; forbidden legacy recommendation text; the backend-neutral `configured-aec` label; and preservation of the additive raw `stimulus_id` while summary, coupling, classification, and exit logic neither validate nor consume it. The adjacent command uses the same prefix/flags with `tests/test_interrupt_suite.py tests/test_echo_probe.py tests/test_barge_self_interrupt.py tests/test_autotest_verdicts.py tests/test_diagnose_run.py -q` and passes `284` in 1.34s; APM/DTD passes `6` in 0.69s. No model, network, microphone, audio device, open-speaker, GPU, physical suite, runtime/config/default/backend, identity authenticity, or live evidence follows (ADR-0176/0179). |
