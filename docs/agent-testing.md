...
| Echo-probe runtime AEC-delay provenance | `SPEAKER_TEST_LOG=0 PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 nice -n 19 ionice -c 3 /home/dobo/work/speaker/.venv/bin/python -B -m pytest -p no:cacheprovider tests/test_echo_probe.py tests/test_aec_seam.py tests/test_denoise.py -q -rs` | `89 passed, 1 skipped`: scalar snapshot/reset provenance; disabled, fail-open, fixed, pending, accepted-equal-seed, accepted-different-seed, impossible-state rejection, non-16-kHz, retained-owner, and additive-consumer compatibility cases pass headlessly; the skip is the absent DTLN ONNX. The adjacent AEC/capture command uses `tests/test_sherpa_media_session.py tests/test_sherpa_streaming_decode_session.py tests/test_input_calibration.py tests/test_aec_seam.py -q -rs` and passes `134, 1 skipped`; APM/DTD separately passes `6`. No model, microphone, audio device, open-speaker, GPU, network, delay quality, ERLE quality, or live evidence follows (ADR-0175). |
