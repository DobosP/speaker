...
`core/websearch.py` registers a `web.search` capability that enforces the order **turn-context/scope/retained-prompt veto → raw-query gate → enabled/backend check → SearXNG → result normalization → corpus fallback**. A context veto, raw-gate denial, disabled search (`web_search.enabled=false` by default, including when a backend was injected), or missing backend returns the corpus with `data["egress"]=False`; a context veto is stamped PRIVATE. If both gates permit, a `SearxngBackend` (lazy `httpx` import) queries `{base_url}/search?q=<query>&format=json` with the configured timeout applied separately to connect/read/write/pool inactivity (BR7, ~4 s each), not as a total wall-clock deadline. Normalization consumes only the configured `max_results` prefix; non-mapping hits are skipped and usable mappings become `{name, summary}` plus citations. Empty/all-skipped results fall back locally with `ok=True` and `egress=true`; a backend or mapping-normalization exception does the same with an error-type receipt, because egress already occurred. Thus a malformed backend hit cannot abort the plan or erase its egress fact.
...
This is defense in depth, not authenticated provenance or process isolation. Same-process provider code or another holder of a context/shallow-value alias can still mutate it outside snapshots; the raw PII classifier remains heuristic. The shipped HTTP backend keeps its transport timeout, but an already-entered blocking injected/custom backend call is not forcibly cancelled, and response parsing, trusted configured `max_results`, plus individual value sizes are not newly hard-capped. ADR-0189's retained-context model/web fence is implemented and verified on the canonical assembled path; the explicit direct/custom/raw and same-process residuals above remain.
