...
- **[`docs/archive/architecture.md`](archive/architecture.md)** — the *as-built* implementation snapshot (superseded 2026-07-02, archived). Its intro on the core shape (one core + many shells, the `AudioEngine` seam, the `AgentEvent`/`Mode` contract) is historical background only.
...
- **DTLN-aec** (`aec_backend='dtln'`): experimental deep ONNX echo canceller (two-stage LSTM, pending tflite→ONNX). Ships with the NumPy FDAF (dependency-free, ~10–20 dB ERLE) as the always-available fallback; DTLN fails open to no-AEC when unavailable. `aec_ref_delay_ms` is the configured seed; runtime auto-delay may replace it after an accepted measurement, and `tools/echo_probe.py` reports both. Turning auto-delay off leaves the seed fixed and requires separate live qualification; it is not the supported self-calibrating path. Full gate inventory in [§9](#9--optional--experimental-tiers-gate-inventory); echo-cancellation defense layers in [§7](#7--real-time-quality-subsystems).
...
│         │  lexical barge-in, optional ID, TTS │         │
│         └─────────────────────────────────────┘         │
...
- **`SherpaOnnxEngine`** — production, on-device: VAD + streaming STT + endpointing + keyword spotting + TTS. Word-cut is enrollment-free by default, with an explicit multi-voice speaker filter and a fail-closed identity tie-breaker for STOP controls that overlap own TTS.
...
1. **Native pre-gain split** (`AudioResampler`, `core/audio_frontend.py`): one independent stateful anti-aliased downsampler feeds startup/recovery calibration and ordinary-final acoustic evidence before application gain. It learns ambient energy plus a normalized 20-band spectrum; invalid calibration abstains. A separate resampler state feeds the live recognizer path, so the evidence tap cannot advance its FIR twice (ADR-0048).
2. **Gain + live resampling** (`InputAGC` or `apply_gain_soft_limit`, then `AudioResampler`): Mic-level scaling with smooth saturation followed by the 16 kHz anti-aliased downsampler. The resampler prefers soxr (state carried across blocks) → scipy polyphase → naive linear diagnostic fallback. Enrollment uses the same capture transform. (Quality rationale in [§7](#7--real-time-quality-subsystems).)
3. **AEC** (acoustic echo cancellation, `core/engines/_aec.py`, **default off**, gated by `sherpa.aec_enabled`): Optional NumPy-backed frequency-domain block adaptive filter (FDAF, 512-tap frame, 50% overlap-save). Takes the near-end (mic) block and the far-end reference (TTS being played, teed via `FarEndRing` from the playback thread and read at the current operating speaker→mic delay) and subtracts the loudspeaker→mic echo before any recognizer/VAD/speaker embedder downstream. `aec_ref_delay_ms` seeds that delay; when `aec_auto_delay` is on, an accepted runtime estimate replaces the seed. `tools/echo_probe.py` reports the configured seed, acceptance state, and final operating samples without rewriting config. Double-talk freeze prevents the filter diverging onto the user's voice during simultaneous talk. Passthrough-on-error. Linear/headset or near-field paths can reach ~10–20 dB ERLE; the nonlinear open laptop measured ~0 dB, so FDAF is not the current open-speaker solution. A deep-learning tier (DTLN-aec, `aec_backend='dtln'`) is reserved but currently fails open to no-AEC, pending tflite→ONNX conversion (see [§7](#7--real-time-quality-subsystems) and [§9](#9--optional--experimental-tiers-gate-inventory)).
...
5. **VAD + streaming ASR + calibrated admission + optional two-pass final** (`sherpa-onnx` zipformer + offline SenseVoice): VAD opens an owned speech epoch and token-by-token ASR produces low-latency hypotheses. Normal partials/finals publish only after the parallel pre-gain branch supplies YIN-style periodicity plus either a dynamic 80/80 ms or steady ≥120 ms calibrated pattern. The typed decision is snapshotted at endpoint and an insufficient final is dropped before async queueing, second-pass ASR, speaker embedding, or runtime dispatch; unavailable calibration fails open. Bounded confirmed-barge/word-cut PCM handoffs bypass only this ordinary admission check (ADR-0048). When `asr_final_backend` is set (`'sense_voice'` — the shipped default — or `'whisper'`), the endpointed utterance is re-transcribed by the offline model for a punctuated final. Absent model or empty backend stays streaming-only. Measured second-pass cost: ~55 ms (2 threads). See [§7](#7--real-time-quality-subsystems) and `docs/archive/asr_two_pass_2026-06-01.md`.
...
**Production OS-EC word-cut** (`core/engines/sherpa.py`): On the verified
echo-cancel route, a novel exact Stop/cancel control or at least four novel
non-own words can interrupt without enrollment. Enrollment alone does not
filter this path; generic multi-voice overrides can require identity with
`sherpa.barge_word_cut_require_speaker=true`, while novel exact Stop/cancel
remains an open fail-safe control. Independently, ADR-0042 keeps
STOP-class controls and attested repairs that overlap current or recent own TTS
...
The LLM stack is two-tiered: a small fast model for snappy spoken replies (local-only, always) and a main model (research, multimodal, reasoning) that **optionally** hedges or falls back to cloud. Current-turn text/file/screen egress is classified by sensitivity (personal/code/public) and routed through failure-tolerant cloud chains. ADR-0189's verified implementation adds typed, monotonic provenance that keeps its enumerated dynamic retained sources on the local main leg and out of canonical web search. Frozen fake/headless receipts pass 262 focused, 275 exact cloud-stage, 101 adjacent, 1,196 broad, 103 resume-integration, 314 import, and 6 APM/DTD tests; exact timings and hashes are recorded below and in the ADR. Raw-audio topology is recorded in ADR-0097.
...
**Worker management (ADR-0190; implemented and headlessly verified):** HedgeLLM publishes one exact owner before constructing/starting each source daemon. Canonical factory Hedges share a process-wide registry: the local main member, single-cloud compatibility member, and each exact named cloud preset keep stable keys across sensitivity chains and rebuilds. Directly constructed Hedges instead have instance-private local/cloud-index keys. The local-only/no-cloud path claims a no-thread lease for the same local key. A BUSY member starts no thread/client call and is skipped immediately so fallback/hedge can progress through distinct keys. Only exact worker return, cleared prompt/system/history/images/context/client/stream aliases, dead-thread proof, zero-time join, and identity-matched registry release free a key; ambiguous thread construction/start or cleanup remains retained. Loser stop and explicitly advertised safe stream cancellation remain cooperative. After advertised nonblocking cancel calls return, all owners share the existing bounded join budget (`WORKER_JOIN_TIMEOUT` 0.5 s), after which the outer provider slot may retire while the durable owner keeps same-source successors BUSY and unrelated keys available. A cancel callback violating that contract can still block before the budget begins; otherwise one uncertain returned-to-coordinator source cannot multiply same-key work, but it can retain its client/model, request context, socket/native/SDK resources, and possible cloud billing until actual return or process exit. This is per-key rather than global: raw/direct aliases and differently named aliases can bypass or partition it. It proves no GIL availability, native return, transport closure, billing cessation, or hard kill. The 30-second winner drain idle timeout and chain-scaled finite winner-selection budget remain; late retired tokens cannot resurrect a source. Frozen fake/headless receipts pass 44 focused, 256 adjacent, 319 current cloud-stage, 157 cancellation/routing adjacent, 315 import, and 6 APM/DTD tests; static checks and independent lifecycle review are GO, with no provider/network/model/GPU/audio/device/live path run.
...
**Retained-context model/web admission (ADR-0189; implemented and verified).** Sensitivity remains a jurisdictional model-chain selector, not retained-context egress authority: current-turn post-ASR PRIVATE text may still use its configured US model chain. Canonical Assistant, research, and ReAct calls additionally carry exact typed `always_on_agent.models.CloudEgressScope` under `CLOUD_EGRESS_SCOPE_CONTEXT_KEY = "cloud_egress_scope"`; `core.llm` re-exports both. Exact member values are `current_turn_only` and `local_only`. Capability entry mints current scope only when the key is absent and never overwrites a present local/malformed restriction. Every factory-managed multi-chain and single-cloud Hedge accepts only exact current scope; raw strings, coercible objects, missing/malformed values, and local scope take the bare local leg before the shared queue, any thread, or any provider-member stream exists. The eager call-time snapshot is monotonic across retry and ReAct.

The compositor downgrades before a model call whenever it includes recent role history, composed/planner recent text, or a nonempty prepublished legacy `recent_conversation`; recall, profile, last-session, or recalled screen-memory text; any nonempty procedural-rule block; or a PRIVATE local/vault-tool finding. ReAct carries the downgrade before its next planning/final call and before a generated web tool call; deterministic research synthesis does so before folding a PRIVATE local finding into its prompt. Procedural instructions may be trusted for behavior but are not cloud-disclosure consent.

Two shared metadata receipts cover retained prompt text outside those blocks. Exact-True `SYNTHETIC_RESUME_TAIL_METADATA_KEY = "synthetic_resume_tail"` marks an accepted `ResumeTracker` prompt because it embeds the prior query plus assistant tail actually spoken; generic post-barge/current-user text has no such receipt. Exact-True `RETAINED_PROMPT_CONTEXT_METADATA_KEY = "retained_prompt_context"` is minted when an accepted substantive transcript-cleaner rewrite used nonempty context from the newest four exact canonical prior-user utterances. Cleaner equivalence applies NFC/casefold, ignores whitespace and ASCII punctuation, and keeps every remaining Unicode content point significant. Thus case/spacing/ASCII-punctuation-only cleanup and context-free substantive rewriting remain eligible. The claim is token/content provenance, not literal byte noninterference.

The Supervisor treats retained-key presence as restrictive, ORs exact True through published-unheard, reservation, extension/coalescing, merge, queued replacement, and folded continuation lineage, copies both reserved receipts into the exact task, then clears ambient turn state. Assistant/research treat retained-key presence, a non-exact metadata container, or non-string metadata keys as local-only while ignoring unrelated exact-string metadata. `web.search` applies the same restrictive shapes before classifier/backend. Absent top-level scope preserves canonical deterministic SEARCH/RESEARCH and direct compatibility; if scope is present, only exact current may proceed to the still-required sensitivity/raw gates. Present local/malformed scope or retained-key presence vetoes to the local corpus, closing public-looking ReAct queries over retained context.

This source-derived fence avoids running the heuristic raw-query classifier over the whole assembled system prompt, whose ordinary static instructions contain possessive language and whose dynamic facts may evade regexes. The outbound high-confidence redactor remains a last-line scrub. Explicit file/image attachments, ambient/current images, public findings already returned through web search, and static configured system/persona/capability-manifest text retain existing policy. The claim is limited to canonical `build_llms` plus `build_runtime`, factory-managed Hedges, and registered `web.search`; direct/custom/raw providers, shallow aliases, and arbitrary same-process mutation remain outside it. The deliberate case/spacing/ASCII-punctuation byte/covert channel, compositor surfaces outside the exact snapshot seams that are not globally hook-free, shared `HedgeLLM.last_source` diagnostic race, and provider-authored explicit error/data remain residuals. ADR-0192 closes only ADR-0189's untagged unexpected registered-provider exception-string residual. Executable current-image coverage exists, but no file-path test is claimed. ADR-0097's media topology is unchanged.

**Capability exception sanitization (ADR-0192; implemented and verified):** `always_on_agent.capabilities.CAPABILITY_PROVIDER_FAILED` is the exact public literal `capability_provider_failed`. If a registered provider raises an ordinary `Exception`, the common registry boundary catches it without binding, inspecting, typing, stringifying, repr-ing, logging, or retaining it. It also admits only `type(result) is CapabilityResult`; a foreign object, proxy, or subclass fails before observer attribute access. Either path returns a fresh exact `CapabilityResult(False, "", error="capability_provider_failed")` with empty data and citations before a finished observer, task step, textual/native ReAct observation, or later otherwise-authorized model prompt can consume detail. An exact normally returned `CapabilityResult` remains unchanged—including explicit error/data—and missing-capability/authority semantics remain. A direct registry `BaseException` reraises unchanged after a paired fixed finish when subscribed. The assembled `TaskRuntime` pops and clears a contained provider `BaseException` and returns the same exact failed result, so ordinary `TASK_FAILED` carries only the bare code with no synthetic exception log. Direct calls, deliberate exact-result detail, prior provider logs/effects/finalizers, observer process controls, unrelated task/controller exceptions, same-process mutation, and provider/resource/billing containment remain outside this boundary. Frozen fake/headless receipts pass 14 focused, 262 adjacent, 319 current cloud-stage, 88 task/cancellation, 315 import, and 6 APM/DTD tests; static/debt checks, exact hashes/inventory, receipt audit, architecture review, and final independent landing audit with 30 relevant tests/hostile probes are GO. No network, provider, model, GPU, audio, microphone, device, billing, or live path ran.

`may_leave_device` is the necessary raw-query gate for the web-search surface (BR3): personal/PII text and MEETING/COMMAND/DICTATION/MEETING_NOTE authority block any network call to the self-hosted SearXNG backend. ADR-0187 adds an earlier, monotonic provider-level veto. `CapabilityRegistry` preserves every supplied context and creates an empty dict only for `None`; observer fields come from an exact-dict snapshot and exact-string keys/values only. ReAct invalidates missing sensitivity, a non-exact Mapping, or a non-string key to an explicit malformed-sensitivity receipt before tool-context copying. The direct `web.search` capability boundary—including canonical deterministic SEARCH/RESEARCH—receives exact-built-in-dict/exact-string-key absent-sensitivity compatibility, and absence merely proceeds to the raw gate; this is distinct from ReAct entry. Exact `private`, every present malformed/unknown value, non-string keys, and dict subclasses deny before virtual container/coercion hooks. Exact `public`/`code` are structural restrictions rather than opaque provenance and cannot declassify the raw query, mode, intent, or classifier-failure verdict. Mode/intent accept real enum members or exact built-in strings; `None`, unknown, and hostile non-string values retain the historical `None` abstention without invoking their hooks. ADR-0189 additionally makes any present top-level model scope and retained-prompt task metadata monotonic web restrictions without changing those gates.

### Web search: SearXNG backend and corpus fallback

`core/websearch.py` registers a `web.search` capability that enforces the order **turn-context/scope/retained-prompt veto → raw-query gate → enabled/backend check → SearXNG → result normalization → corpus fallback**. A context veto, raw-gate denial, disabled search, or missing backend returns the corpus with `egress=false`; the provider never returns `ok=False`. Web access is default-off and requires exact built-in `True` plus a nonempty exact-string `base_url`. Direct and JSON config paths share the same finite defaults and clamps: phase timeout `4.0` (`0.05..15`), cooperative total `8.0` (`0.05..20`), body bytes `262144` (`1024..1048576`), results `5` (`1..8`), title `256` (`1..512`), summary `2048` (`1..4096`), URL `2048` (`1..2048`), and output `16384` (`1..32768`). Numeric booleans/subclasses are not accepted; malformed/non-finite values default.

ADR-0191 directly pins `httpx==0.28.1`. After every egress gate passes, the exact shipped `SearxngBackend` lazily opens a context-managed GET to `{base_url}/search` with `q` and `format=json`, an HTTPX connect/read/write/pool inactivity timeout, JSON Accept, identity encoding, and raw chunk iteration. It requires exactly one JSON content type, absent/single identity encoding, absent/single decimal content length within the cap, exact-byte chunks whose accumulation remains within the cap, strict UTF-8 without BOM, and strict JSON with duplicate-key/non-finite/recursion-overflow refusal. The root must be an exact object with a results list. Whole bounded JSON parsing precedes validation/copy of only `results[:max_results]`; consumed shipped hits are exact dicts with missing/null/exact-string fields.

The provider consumes at most the configured prefix from shipped or custom iterables, skips non-exact/hostile hit/key/field shapes without coercion hooks, caps title/summary/URL before building results/citations, and separately caps rendered output. Exact `threading.Event` cancellation and a monotonic total deadline are checked before transport and whenever synchronous control returns around enter/raw-read/end/close/decode/JSON/prefix work. A pre-attempt cancellation/deadline remains `egress=false`; transport-factory invocation, entered-response failures, malformed replies, empty results, and custom-backend failures remain truthfully post-attempt. Logs and receipts contain only stable shipped codes or exception type names, never query/body/exception detail. Exact-type dispatch preserves the legacy one-argument protocol for custom backends and `SearxngBackend` subclasses; they do not inherit shipped HTTP bounds.

These are bounded ingestion contracts, not a global wall-time, memory, provenance, or process-isolation proof. Phase timeouts and cooperative checkpoints cannot preempt synchronous factory/enter/read/exit or active JSON parse. The body cap excludes DNS/TLS/HTTP header and proxy/client allocation, one raw transport chunk before rejection, bytearray/bytes/text copies, and parsed-graph amplification; whole JSON is parsed before prefix normalization. Field/output caps count Python code points and citations are bounded indirectly by `max_results`; escaped lone UTF-16 surrogates remain bounded but are not Unicode-scalar-sanitized or downstream-encoding-proof. The exact Event is trusted same-process state, not a hostile-hook boundary. Custom backends may block, allocate, mutate, or egress arbitrarily; direct backend calls bypass provider privacy gates. The configured endpoint and environment proxy remain trusted, and no loopback/TLS/origin/certificate/proxy-isolation guarantee is added. Same-process aliases/mutation and the heuristic raw-classifier residual remain. ADR-0189's canonical retained-context fence is unchanged.

ADR-0191's final low-priority/no-cache fake/headless gates pass focused `136 in 0.59s` (`real 0.77s`, `user 0.53s`, `sys 0.03s`), adjacent `186 in 0.84s` (`real 1.01s`, `user 0.71s`, `sys 0.05s`), sensitivity/LLM-egress `108 in 0.57s` (`real 0.75s`, `user 0.61s`, `sys 0.04s`), imports `315 in 2.02s` (`real 2.31s`, `user 2.16s`, `sys 0.13s`), and APM/DTD `6 in 0.81s` (`real 1.02s`, `user 0.88s`, `sys 0.13s`). Config JSON, AST2, exact HTTPX pin, scoped Ruff, diff-check, core format, five inherited/base-identical test formatter hunks with zero task-line overlap, STATUS100, frozen hashes, exact ten-path inventory, and independent rebased code/security audit are green. No network, SearXNG, provider/model, GPU, audio, microphone, device, or live path ran.

ADR-0189's frozen low-priority/no-cache fake/headless gates pass focused changed-surface `262 in 6.09s` (`real 6.24s`, `user 1.31s`, `sys 0.20s`), exact registered cloud-stage `275 in 9.91s` (`real 10.08s`, `user 1.14s`, `sys 0.13s`), declared-adjacent `101 in 1.57s` (`real 1.73s`, `user 0.74s`, `sys 0.06s`), broad policy/context/ReAct/vault `1196 in 1.91s` (`real 2.09s`, `user 1.62s`, `sys 0.20s`), final-preprocessing/post-barge resume integration `103 in 7.77s` (`real 7.92s`, `user 1.09s`, `sys 0.18s`), imports `314 in 1.88s` (`real 2.15s`, `user 1.94s`, `sys 0.13s`), and APM/DTD `6 in 0.72s` (`real 0.90s`, `user 0.78s`, `sys 0.11s`). AST13, scoped Ruff `E9,F63,F7,F82`, diff-check, STATUS100, pre/post hashes, and the exact 19-path inventory are green; all 13 Python hashes are in ADR-0189, and security plus architecture/adversarial review are GO. Managed policy rejected three task-owned mechanical format suggestions documented in the ADR, so no bytes changed and no full-format-clean claim follows. No network, SearXNG, cloud-provider, model, GPU, audio, microphone, device, or live path ran.

Frozen low-priority headless gates pass 96 focused, 111 policy/context-adjacent, 161 capability/observer-adjacent, 150 ReAct/untrusted/cancellation-adjacent, and 6 APM/DTD. Six-file AST parsing and diff-check are green; scoped Ruff is green while ignoring React's identical inherited F401, and all 37 remaining formatter hunks are base-identical with zero task-line overlap. No network, SearXNG, cloud/model, audio, device, or live path ran.
...
Each turn, the capability layer publishes its sensitivity via `capability_context` (a `ContextVar`); `SensitivityRouterLLM._pick()` reads it and returns the corresponding `HedgeLLM` backing. The `llm.cloud_routing.default_chain` (default `"private"`, fail-safe) is used when sensitivity is empty or the selector returns an unknown chain. Separately, a factory-managed Hedge must see the exact typed `CloudEgressScope` member whose value is `current_turn_only` before any cloud member can start; missing, malformed, or `local_only` scope takes the bare local leg regardless of the selected chain (ADR-0189; implemented and frozen).
...
3. **Acoustic Echo Cancellation (optional in-app fallback):** When `aec_enabled=true`, `core/engines/_aec.py` subtracts the played TTS (the far-end reference, teed from playback into a `FarEndRing` and read at the current operating speaker→mic delay) from the mic block *before* any consumer. **The in-app open-speaker fallback is WebRTC APM** (`aec_backend='apm'`: AEC3 + residual-echo suppression + NS + AGC2 + HPF; selected by the separate `open_speaker` profile, `apm_always_on=true`) — it tolerates the nonlinear open laptop speaker where the linear NLMS filter measures ~0 dB ERLE (**NLMS live-ruled-out for open-speaker 2026-06-17**; see [`docs/audio_pipeline.md`](audio_pipeline.md) for the full capture chain). Do not combine this profile with `./live.sh`, whose normal Linux path uses OS EC. The dependency-free **NumPy FDAF/NLMS** (frequency-domain block adaptive filter, 512-tap frame, 50% overlap-save; ~10–20 dB real-world ERLE) remains the *base* default for headset / near-field rooms only. A **deep DTLN-aec ONNX tier** (`aec_backend='dtln'`) is reserved but currently fails open to no-AEC pending tflite→ONNX conversion (no models shipped). Double-talk freeze stops the filter diverging onto the user's voice. `aec_ref_delay_ms` is the seed; `aec_auto_delay` may accept and apply a runtime value, which `tools/echo_probe.py` reports without changing config. Never hardcode 260 ms. `aec_filter_taps` remains per-device tuning. When AEC is on, the gate uses the smaller `aec_relaxed_margin_db` (3.0 dB) instead of `barge_in_output_margin_db`.
...
2. **Echo probe ERLE/runtime-delay validation** — on each real device/room, use `tools/echo_probe.py` to verify whether auto-delay accepted a runtime operating value and whether measured ERLE plus live barge behavior are safe. `aec_ref_delay_ms` (default 80 ms) is only the seed on the supported self-calibrating path; turning auto-delay off leaves it fixed and requires separate live qualification. `aec_filter_taps` (default 512) remains per-device tuning, and the 6 dB `barge_in_output_margin_db` level fallback is validated on one Realtek laptop. **(M effort, requires field data + echo-probe measurements)**
...
**An append-only chronological record of locked architectural and implementation decisions, including rationale.** Later phases may supersede earlier entries; the live authority for open items lives in `docs/archive/ultracode_scope.md` (phase pipeline) and `docs/target_architecture.md` (structural decisions). Resolved P0–P5 scope, P2 R1–R12 revisions, P3 BR1–BR8 revisions, and current-session bindings are collected here so archived source docs can retire without losing context.
...
23. **Planner + ReAct complementary, not duplicates.** `always_on_agent/planner.py` (explicit TaskPlans) and `always_on_agent/react.py` (bounded ReAct loop with tools) are **both loaded and active** — the planner routes to react for RESEARCH/ACT; react supplies the DEFAULT_TOOLS fallback. *Rationale:* clean task scoping; planner explicit, react the bounded-exploration fallback. VERIFIED: `always_on_agent/tasks.py:586-588` branches on the plan step's `ok`; react tool invocation remains controller-owned. (2026-05-29). (See [§2](#2--the-control-plane-brain-always_on_agent).)
...
30. **Coherence barge-in confirmation: "a bit slower, higher confidence" (2026-06-02).** The interrupt now requires `coherence_confirm_frames` (default **2**, ~0.1 s each) CONSECUTIVE over-threshold frames before `EchoCoherenceDetector.decide()` fires; the detector class default stays 1 (fire-on-first-frame) so existing unit behavior is unchanged. A single over-threshold spike — a cheap speaker's nonlinear distortion or transient noise momentarily crossing the bar — no longer self-interrupts; a sustained talk-over still does. While the run builds, `decide()` returns False (not None), so an unconfirmed moment never falls through to the identity/level gate, and the EWMA chart is updated only on below-threshold frames (an unconfirmed candidate can't drag it). *Rationale:* the user asked to make the interrupt "a bit slower but with higher confidence" and to keep it independent of user/speaker detection — this hardens the enrollment-free coherence signal directly, costing ~one extra block of latency for a large drop in single-frame false fires; speaker-ID remains a separate, abstain-time fallback only.

### Open-speaker AEC decisions (2026-06-17 / 2026-06-21) — follow-up entries added 2026-07-02
