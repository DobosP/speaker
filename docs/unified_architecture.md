...
This is defense in depth, not an authenticated context-provenance or process-isolation boundary. Same-process provider code or another holder of a context/shallow-value alias can still mutate it outside the snapshots; the raw PII classifier remains heuristic and can miss private phrasing. The shipped HTTP backend keeps its transport timeout, but an already-entered blocking injected/custom backend call is not forcibly cancelled, and transport response parsing, trusted configured `max_results`, plus individual result-value sizes are not newly hard-capped. The separate PRIVATE cloud-LLM injected-PII gap remains open.
