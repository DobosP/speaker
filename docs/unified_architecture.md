...
13. **Sensitivity egress gate (§9.7, BR3).** New `core.sensitivity.may_leave_device(query, *, mode, intent_kind)` predicate: **PRIVATE (PII/possessive) blocks egress; COMMAND/DICTATION/MEETING intents block egress; else permit to SearXNG.** Guarded enum coercion (BR2 HIGH); fail-safe to PRIVATE on a bad `context["mode"]`/`intent_kind`. PRIVATE ⇒ corpus fallback with `egress=False`. *Rationale:* preserves the §9.7 boundary; blocks credential/personal egress; permits plain public queries ("weather in Berlin") to the self-hosted lookup. `docs/archive/p3_design.md` BR2, BR3 (2026-05-29).
