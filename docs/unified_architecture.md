...
ADR-0191's fake/headless gates, re-verified on main `523f22f` (2026-09-07), pass focused `136`, adjacent `186`, sensitivity/LLM-egress `108`, imports `322`, and APM/DTD `6`; the frozen 2026-08-12 timings are in the ADR. Config JSON, AST2, exact HTTPX pin, scoped Ruff, diff-check, core format, five inherited/base-identical test formatter hunks with zero task-line overlap, frozen hashes, exact ten-path inventory, and independent rebased code/security audit are green. No network, SearXNG, provider/model, GPU, audio, microphone, device, or live path ran.
