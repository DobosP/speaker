...
- **DTLN-aec** (`aec_backend='dtln'`): experimental deep echo canceller. Ships with the NumPy NLMS/FDAF AEC as the always-available fallback. Full gate inventory in [§9](#9--optional--experimental-tiers-gate-inventory); echo-cancellation defense layers in [§7](#7--real-time-quality-subsystems).
- **Smart Turn v3** (`SmartTurnCompletionDetector`): experimental prosodic endpointer. Ships with the lexical detector as the baseline. Smart Turn is gated off by default. Endpointing detail in [§7](#7--real-time-quality-subsystems).
...
3. **AEC** (acoustic echo cancellation, `core/engines/_aec.py`, **default off**, gated by `sherpa.aec_enabled`): Optional NumPy-backed frequency-domain adaptive filter (FDAF/fast LMS). Takes the near-end (mic) block and the far-end reference (TTS being played, teed via `FarEndRing` from the playback thread) and subtracts the loudspeaker→mic echo before any recognizer/VAD/speaker embedder downstream. Passthrough-on-error. Achieves ~10–20 dB ERLE on real open speakers; good enough for headsets and moderate rooms. A deep-learning tier (DTLN-aec) is gated, pending ONNX conversion (see [§7](#7--real-time-quality-subsystems) and [§9](#9--optional--experimental-tiers-gate-inventory)).
...
5. **VAD + streaming ASR** (`sherpa-onnx` zipformer): Voice activity detection, token-by-token streaming transcription, and partial/final emission via callbacks.
6. **Speaker embedder** (enrolled speaker gate, see below).
7. **Barge-in gate** (see below).

### Endpointing & barge-in

**Semantic turn-completion** (`core/endpointing.py`): Layered on top of the acoustic silence timer (configurable `asr_rule2_min_trailing_silence`, default 0.8 s). A pluggable `TurnCompletionDetector` protocol (shipped: `LexicalTurnCompletionDetector`, deterministic lexical analysis; optional `SmartTurnCompletionDetector` prosodic ONNX). When the partial reads as a complete turn, fire `on_final` early; when it ends mid-phrase ("and", "the", "um"), hold past the timer (up to `endpoint_max_silence_sec`). Falls back to pure acoustic if disabled. Full mechanics in [§7](#7--real-time-quality-subsystems).

**Speaker-gate barge-in** (`SpeakerGate`, `core/engines/speaker_gate.py`): Optional speaker-ID gate that compares a detected voice embedding against an enrolled reference (`core/enroll.py`; persisted as JSON next to models). Enrollment is one-time (`python -m core --enroll`) and averages multiple passes. Thresholds via cosine similarity (default 0.5). Paired with the speaker embedder in the front-end, it gates barge-in on speaker identity so the assistant's own TTS and nearby ambient speech don't self-interrupt.

**AEC + output-margin defense:** Three layers in order of strength: (1) speaker-ID gate (when enrolled); (2) AEC + relaxed output margin; (3) bare output-margin guard (`passes_output_margin`). Headsets sidestep echo entirely. The full three-layer defense and calibration data are detailed in [§7](#7--real-time-quality-subsystems).
...
Real-time latency and speech quality depend on stacked subsystems: **semantic turn-completion endpointing** (commit a final when the user finishes, not after a fixed acoustic timer), the **never-stuck controller** (reap hung tasks and heal on the watchdog tick), **startup pre-warm** (load models before turn 1), **barge-in with echo cancellation and speaker-ID** (three-layer defense against self-interruption), and **STT input-chain quality** (anti-aliased resampling, soft-limit AGC, and optional confidence gating).
...
- **SHORTEN:** when the partial reads as a complete turn, commit early (down to `endpoint_min_silence_sec`, shipped 0.7 s per on-device validation). The latency win: ~0.8 s → ~0.5 s final wait.
...
The **shipped v1 detector** is `LexicalTurnCompletionDetector` — cheap, deterministic, no model: a normalized last-word check against `DEFAULT_INCOMPLETE_ENDINGS` (coordinating conjunctions, articles, fillers, EN + RO). Deliberately conservative: a false-SHORTEN (early commit) is recoverable via the continuation layer (see [§2](#2--the-control-plane-brain-always_on_agent)); a false-EXTEND is not. The protocol also declares `needs_audio`, so the **optional Smart Turn v3 ONNX** (pipecat-ai, ~8 MB, Whisper-tiny log-mel + sigmoid head, ~12 ms CPU) drops in without re-architecture when `smart_turn_enabled=true` + path + `endpoint_enabled=true`. Both detectors are gated: disabled by default; validated on-device before shipping (gate inventory in [§9](#9--optional--experimental-tiers-gate-inventory)).
...
### Barge-in: three-layer defense against self-interruption

The assistant keeps the mic open during playback to hear user talk-over. Without AEC, its own TTS leaks from speakers into the mic and looks like a barge-in — the assistant cuts itself off. Three layers, strongest first:

1. **Speaker-ID identity gate** (when enrolled): The `SpeakerGate` (`core/engines/speaker_gate.py`) compares a speaker embedding of detected speech against the enrolled user's voice via cosine similarity. Only a match above `threshold` (0.5) counts as barge-in. Run `python -m core --enroll` to load an enrollment; the gate then rejects the assistant's echo by *identity*, independent of volume. When unenrolled, the gate fails open.
2. **Acoustic Echo Cancellation (optional):** When `aec_enabled=true`, `core/engines/_aec.py` subtracts the played TTS (the far-end reference, teed from playback into a `FarEndRing` thread-safe buffer and read at the configured speaker→mic delay) from the mic block *before* any consumer. The shipped backend is the **dependency-free NumPy FDAF** (frequency-domain adaptive filter, constrained, normalized, 50% overlap-save): real-time, cross-platform, ~10–20 dB real-world ERLE on headsets / near-field rooms. A **deep DTLN-aec ONNX tier** can drop in for louder rooms (currently gated: `aec_backend='dtln'` fails open to no-AEC pending tflite→ONNX conversion). Double-talk freeze stops the filter diverging onto the user's voice during talk-over. `aec_ref_delay_ms` (calibrate with `tools/echo_probe.py`) and `aec_filter_taps` tune the cancellation per device. When AEC is on, the barge-in gate uses the smaller `aec_relaxed_margin_db` (3.0 dB) instead of `barge_in_output_margin_db`.
3. **Output-margin guard (unenrolled fallback):** When the speaker gate is unenrolled and no AEC is on, `SpeakerGate.accept()` gates on level: detected speech must sit `barge_in_output_margin_db` dB (shipped 6.0) *above* the current playback-buffer RMS to count as a barge-in. This is device-specific (the gate compares mic-capture RMS against playback-buffer RMS — different scales, volume-dependent). The calibration was run on a Realtek laptop mic (`tools/echo_probe`): at 0 dB self-interruptions fire 15–21× at every volume; at 6 dB they drop to 0 across 30–100% OS volume. Re-run the probe on other hardware or prefer enrollment for a volume-independent fix. When playback is silent (`playback_level ≤ 0`), the gate fails open so a real interrupt is never lost.

### STT input-chain quality

The biggest WER leaks are not the model — they're in the input chain before the recognizer. `core/audio_frontend.py` + capture-loop integration:
...
| **Input Gate** (addressing: ACT vs INGEST) | `input_gate.enabled` | OFF (desktop: ON) | `tests/test_speaker_input_gate.py` | KEEP, gated |
| **Smart Turn v3** (prosodic endpoint) | `sherpa.smart_turn_enabled` + `sherpa.smart_turn_model` | OFF | `tests/test_smart_turn_endpoint.py` | KEEP, gated |
...
All nine tiers are **KEPT as load-bearing experimental gates**. Each yields **byte-identical behaviour when disabled** — the existing code path runs unchanged, no overhead. The runtime wires each gate via a deterministic configuration block + optional env override:
...
- **Smart Turn v3** (`core/endpointing.py` + `sherpa.smart_turn_enabled` + `sherpa.smart_turn_model`): Dual-gated — requires BOTH the flag AND a model path AND `endpoint_enabled=true`. Lazy imports (onnxruntime + transformers) cost nothing on phone; a load error falls back to the lexical detector. Off by default. (Endpointing detail in [§7](#7--real-time-quality-subsystems).)
- **DTLN-aec** (`core/engines/_aec.py`): `aec_backend='dtln'` defers the ONNX tier (no tflite→ONNX conversion yet); currently fails open to no-AEC. The default `'nlms'` ships the dependency-free NumPy FDAF, which is byte-identical when `aec_enabled=false`. (Echo defense in [§7](#7--real-time-quality-subsystems).)
...
- **DTLN-aec** (`aec_backend='dtln'`) and **Smart Turn v3** (`SmartTurnCompletionDetector`) are both implemented and gated behind default-off flags. DTLN currently fails open to the NumPy NLMS AEC (always available, no new dependencies) because the tflite→ONNX conversion is pending. Smart Turn is disabled in base config and unvalidated on hardware; the lexical detector (`LexicalTurnCompletionDetector`) ships as the default (verified `core/endpointing.py`).
...
3. **Smart Turn v3 on-device validation is pending.** The model is gated and shipped (never breaks capture — falls back to lexical), but has never been A/B tested on real hardware; marked as experimental in config comments.
...
- ⏸️ **Semantic end-of-turn model** — Smart Turn v3 ONNX built and gated; never A/B validated. Measured endpoint cost is now visible (~0.6 s average via the true SPEECH_END stamp); Smart Turn promises a ~0.3–0.55 s reduction.
- ⏸️ **Acoustic echo cancellation** — NumPy NLMS AEC is the shipped fallback; DTLN-aec is gated but pending tflite conversion. The desktop path has no AEC by default (the 6 dB margin guard + speaker-ID gate remain the stopgaps; see [§7](#7--real-time-quality-subsystems)). Mobile does enable platform AEC (`mobile/lib/assistant.dart`).
...
1. **Semantic endpoint (Smart Turn v3) on-device A/B validation** — the single biggest real latency win (~0.3–0.55 s) and the only way to test mid-thought cut-off reduction. Requires hardware recordings + prompt instrumentation. **(M effort, blocked on hardware access)**
2. **Echo probe ERLE calibration** — the NLMS AEC needs per-device/room tuning for the dB-margin fallback. Currently 6 dB is a single global default validated on one Realtek laptop. **(M effort, requires field data)**
...
17. **Smart Turn v3 (`SmartTurnCompletionDetector`) kept, gated, default-off.** The prosodic turn-completion model (Whisper-tiny log-mel + sigmoid, ~8 MB ONNX, ~12 ms CPU) replaces the cheap lexical endpoint when enabled. Dual-gated: needs `endpoint_enabled=true` AND `smart_turn_enabled=true` AND a model path; otherwise the lexical detector is used. Requires onnxruntime + transformers (lazy import, desktop-only load-error fallback). *Rationale:* the semantic endpoint is experimental; the dual-gate + fallback prevents capture-path breakage. Current `config.json` shows `smart_turn_enabled: false` (default off); `endpoint_enabled: true` (base endpoint on lexical); model path in `config.local.json`.
