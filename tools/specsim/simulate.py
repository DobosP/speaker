...
a :class:`MachineSpec`, compute the timeline of an ``ASR -> LLM -> TTS`` turn and
the user-perceived latencies. This mirrors how ``core``'s capabilities collect
the *full* LLM stream before emitting one ``TTS_REQUEST`` (so TTS starts after
...
SCENARIOS: tuple[Scenario, ...] = (
    ...
    Scenario("research", "Longer request, paragraph-length answer", user_words=9, reply_words=75),
...
def simulate_turn(spec: MachineSpec, scenario: Scenario) -> TurnResult:
    ...
    add("LLM ttft", "ttft", ttft)
    add("LLM generate", "gen", gen)
